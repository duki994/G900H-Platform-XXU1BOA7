...
class SnapScrollController {
    ...
    private int mSnapBound = SNAP_BOUND;
        ...
        switch(event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (!isScaleGestureDetectionInProgress &&
                    ...
                    if (xDiff > mSnapBound && yDiff < mSnapBound) {
                        mSnapScrollMode = SNAP_HORIZ;
                    } else if (xDiff < mSnapBound && yDiff > mSnapBound) {
    ...
    private void calculateChannelDistance(Context context) {
        ...
        mSnapBound = SNAP_BOUND;//(int)(ViewConfiguration.get(context).getScaledTouchSlop() / metrics.density);
        ...
        //if (metrics.density > 2.f) {
        //    mChannelDistance = (mChannelDistance * 1.75f) / metrics.density;
        //} else {
