...
void SetContentCommandLineFlags(int max_render_process_count,
 ...
 //Disabled Zero copy
  parsed_command_line->AppendSwitch(cc::switches::kEnableMapImage);
...
#if defined(DISABLE_WEBGL) && defined(DISABLE_ANGLE)
  parsed_command_line->AppendSwitch(switches::kDisableGLSLTranslator);
#endif

#if defined(OS_ANDROID)
  // Disable texture4444 due to the quality degradation
  // for both of low-end and high-end devices.
#if !defined(ENABLE_4444_TEXTURE)
  parsed_command_line->AppendSwitch(cc::switches::kDisable4444Textures);
#endif

  // If we don't use 4444 textures, then we will need more memory.
  // In that case, we expand memory limit by default in order to avoid flickering.
  if (parsed_command_line->HasSwitch(cc::switches::kDisable4444Textures))
    parsed_command_line->AppendSwitch(switches::kExpandMemoryLimitOnLowEndDevice);
#endif
