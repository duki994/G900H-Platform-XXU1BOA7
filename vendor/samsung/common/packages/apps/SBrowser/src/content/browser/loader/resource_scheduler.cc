...
static size_t kMaxNumDelayableRequestsPerClient = 20;
static size_t kMaxNumDelayableRequestsPerHost = 16;
#else
...
#if defined(S_NETWORK_CONNECTION_COUNT)
void ResourceScheduler::SetMaxNumDelayableRequestsPerClient(int num) {
  if (num >= 10 && num <= 20)
    kMaxNumDelayableRequestsPerClient = (size_t)num;
}

void ResourceScheduler::SetMaxNumDelayableRequestsPerHost(int num) {
  if (num >= 6 && num <= 16)
    kMaxNumDelayableRequestsPerHost = (size_t)num;
}
#endif
}  // namespace content
