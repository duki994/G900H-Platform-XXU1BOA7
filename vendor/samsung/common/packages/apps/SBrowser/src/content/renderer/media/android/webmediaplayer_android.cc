...
void WebMediaPlayerAndroid::load(LoadType load_type,
  ...
  if (player_type_ != MEDIA_PLAYER_TYPE_URL) {
    ...
    if (player_type_ == MEDIA_PLAYER_TYPE_MEDIA_SOURCE) {
      ...
      manager_->Initialize(
            player_type_, player_id_, url, frame_->document().firstPartyForCookies(), demuxer_client_id);
...
void WebMediaPlayerAndroid::DidLoadMediaInfo(
  ...
  manager_->Initialize(
      player_type_, player_id_, redirected_url, frame_->document().firstPartyForCookies(), 0);  
