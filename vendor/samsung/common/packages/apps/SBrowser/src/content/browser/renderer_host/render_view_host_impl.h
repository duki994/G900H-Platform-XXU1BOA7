...
class CONTENT_EXPORT RenderViewHostImpl
 ...
 public:
  ...
  virtual void SetTextZoomFactor(float factor) OVERRIDE;
  ...
  void SetExtraSiteInstance(SiteInstance* extra_site_instance);
#endif
  ...
  void SavePageAs(const base::FilePath::StringType& pure_file_name);
  void OnReceiveBitmapFromCache(const SkBitmap& bitmap);
  // NOTE: Do not add functions that just send an IPC message that are called in
  // one or two places. Have the caller send the IPC message directly (unless
  // the caller places are in different platforms, in which case it's better
  // to keep them consistent).
  void LoadDataWithBaseUrl(const std::string& data, const std::string& base_url,
 ...
 protected:
  ...
  void OnSetBingAsCurrentSearchDefault();
  void OnTakeFocus(bool reverse);
  void OnFocusedNodeChanged(bool is_editable_node,bool is_select_node,
                                              long node_id);
  ...
  void HandleSelectionDrop(int x, int y, base::string16& text) OVERRIDE;
  void HandleSelectionDropOnFocusedInput(base::string16& text, int dropAction) OVERRIDE; 
  void GetFocusedInputInfo() OVERRIDE;
  void OnSelectedMarkup(const base::string16& markup) OVERRIDE;
  void GetSelectionMarkup() OVERRIDE;
  void OnSelectionVisibilityStatusReceived(bool isVisible) OVERRIDE;
  void OnUpdateSelectionRect(const gfx::Rect& selectionRect) OVERRIDE;
  void OnPointOnRegion(bool isOnRegion) OVERRIDE;
  void OnSelectedBitmap(const SkBitmap& bitmap) OVERRIDE;
  void OnRecognizeArticleResult(std::string reader_result_str) OVERRIDE;

  //SBROWSER_FORM_NAVIGATION
  void moveToPrevInput() OVERRIDE;
  void moveToNextInput() OVERRIDE;
  ...
  void OnUpdateTouchedFixedElementHeight(int height);
  ...
  void GetSelectionMarkupWithBounds() OVERRIDE; // MULTI-SELECTION 
#endif
 private:
...
#if defined(SBROWSER_QC_OPTIMIZATION_PRECREATE_RENDERER)
  SiteInstance* extra_site_instance_;
#endif
