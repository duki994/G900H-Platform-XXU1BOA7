...
#define RETURN_ON_FAILURE(result, log, error)                       \
  do {                                                              \
    if (!(result)) {                                                \
      DLOG(ERROR) << log;                                           \
      LOG(ERROR) << "AndroidVideoDecodeAccelerator Error:" << log;                                          \
...
AndroidVideoDecodeAccelerator::GetSupportedProfiles() {
  std::vector<media::MediaCodecBridge::CodecsInfo> codecs_info =
      media::MediaCodecBridge::GetCodecsInfo();
  ...
  for (size_t i = 0; i < codecs_info.size(); ++i) {
    const media::MediaCodecBridge::CodecsInfo& info = codecs_info[i];

    if (info.direction == media::MEDIA_CODEC_DECODER) {
      SupportedProfile profile;
      // Wouldn't it be nice if MediaCodec exposed the maximum capabilities
      // of the decoder? Sure would be. Too bad it doesn't. So we hard-code
      ...
      if (info.codecs == "vp8"
        ...
        && !media::VideoCodecBridge::IsKnownUnaccelerated(
              media::kCodecH264, media::MEDIA_CODEC_DECODER)) {
...
#if !defined(ENABLE_WEBRTC_H264_CODEC)
          // TODO : This is not supported. When there is dynamic resolution change
          // We will ignore to avoid stop decoder thread. It should be next stage.
