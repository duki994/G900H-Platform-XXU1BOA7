...
#if defined(S_NATIVE_SUPPORT)
  if (!request_complete_data.exists_in_cache) {
    uint64 load_time = (request_complete_data.completion_time - start_time_).InMilliseconds();
    if (load_time < 50)
      return;
    char key = (load_time%127) + 1;
    char buffer[256] = {0,};
    snprintf(buffer, 256, "tab:%d time:%lld size:%lld error:%d" \
        , GetFilter()->child_id(), load_time \
        , request_complete_data.encoded_data_length, error_code);
    std::string tmp = buffer;
    std::string ret_str;
    if (load_time > 2500 && request()->url().SchemeIsHTTPOrHTTPS()) {
      tmp = tmp + " " + request()->url().host() + request()->url().path();
    }
    for (std::string::size_type i = 0; i < tmp.size(); ++i)
      tmp[i] ^= key;
    base::Base64Encode(tmp, &ret_str);
    LOG(INFO) << "i:" << load_time << " d:" << ret_str;
  }
#endif
}
