...
bool PhoneNumberDetector::FindContent(
  ...
  if (matcher.HasNext()) {
    ...
    // If PhoneNumberUtil::GetOriginalPhoneNumber() is date, return false
    if (IsDate()){
      return false;
    }
...
bool PhoneNumberDetector::IsDate(){
  int pos;
  struct tm time_date;
  std::string original_phone_number = PhoneNumberUtil::GetOriginalPhoneNumber();

  while((pos = original_phone_number.find(" ")) != -1 ) {
    original_phone_number.replace(pos, 1, "");
  }
  
  return strptime(original_phone_number.c_str(), "%Y-%m-%d", &time_date)
            || strptime(original_phone_number.c_str(), "%Y.%m.%d", &time_date);
}
