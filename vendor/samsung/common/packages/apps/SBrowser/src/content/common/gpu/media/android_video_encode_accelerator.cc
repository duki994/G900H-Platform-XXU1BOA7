...
#if defined(ENABLE_WEBRTC_H264_CODEC)
void AndroidVideoEncodeAccelerator::SendSpsPpsData(bool key_frame) {
  if (!client_ptr_factory_.GetWeakPtr() ||available_bitstream_buffers_.empty()) {
    return;
  }

  media::BitstreamBuffer bitstream_sps_buffer = available_bitstream_buffers_.back();
  available_bitstream_buffers_.pop_back();
  scoped_ptr<base::SharedMemory> sps_pps_shm(
      new base::SharedMemory(bitstream_sps_buffer.handle(), false));
  RETURN_ON_FAILURE(sps_pps_shm->Map(bitstream_sps_buffer.size()),
                    "Failed to map SHM",
                    kPlatformFailureError);
  RETURN_ON_FAILURE(h264_sps_pps_size <= sps_pps_shm->mapped_size(),
                    "Encoded buffer too large: " << h264_sps_pps_size << ">"
                                                 << sps_pps_shm->mapped_size(),
                    kPlatformFailureError);
  memcpy(sps_pps_shm->memory(), h264_sps_pps_buffer, h264_sps_pps_size);
  base::MessageLoop::current()->PostTask(
    FROM_HERE,
    base::Bind(&VideoEncodeAccelerator::Client::BitstreamBufferReady,
             client_ptr_factory_.GetWeakPtr(),
             bitstream_sps_buffer.id(),
             h264_sps_pps_size,
             key_frame));
}
#endif
