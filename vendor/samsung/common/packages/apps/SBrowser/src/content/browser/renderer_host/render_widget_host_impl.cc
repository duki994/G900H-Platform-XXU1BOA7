...
void RenderWidgetHostImpl::WasShown() {
  LOG(INFO) << "RenderWidgetHostImpl::WasShown() :: is_hidden_ : "<<is_hidden_;
  if (!is_hidden_)
    return;
  is_hidden_ = false;
...
void RenderWidgetHostImpl::WasResized() {
  ...
  view_->DidViewPortSizeChanged(new_size);
#endif
...
void RenderWidgetHostImpl::OnOverscrolled(
  ...
  if (view_)
    view_->OnOverscrolled(accumulated_overscroll, latest_overscroll_delta,
                          current_fling_velocity, causal_event_viewport_point);
}
