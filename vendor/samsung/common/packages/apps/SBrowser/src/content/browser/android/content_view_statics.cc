...
class SuspendedProcessWatcher : public content::RenderProcessHostObserver {
 public:
  ...
  virtual void RenderProcessExited(content::RenderProcessHost* host,
    ...
    if (host) {
      LOG(INFO) << "RenderProcessExited: called [host ID = "<<host->GetID()<<"]";
      StopWatching(host);
    }
  }
  ...
  virtual void RenderProcessHostDestroyed(
    ...
    if (host) {
      LOG(INFO) << "RenderProcessHostDestroyed: called [host ID = "<<host->GetID()<<"]";
      StopWatching(host);
    }
  }
  ...
  void SuspendWebKitSharedTimers() {
    ...
    for (content::RenderProcessHost::iterator i(
      ...
      if (host) {
        ...
        LOG(INFO) << "SuspendWebKitSharedTimers: Send IPC [host ID = "<<host->GetID()<<"]";
  ...
  void ResumeWebkitSharedTimers() {
    for (std::vector<int>::const_iterator it = suspended_processes_.begin();
      ...
      if (host) {
        ...
        LOG(INFO) << "ResumeWebkitSharedTimers: Send IPC [host ID = "<<host->GetID()<<"]";
