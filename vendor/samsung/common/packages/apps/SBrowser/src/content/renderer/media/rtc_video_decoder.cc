...
#if defined(ENABLE_WEBRTC_H264_CODEC)
      if (codec_info->codecType == webrtc::kVideoCodecH264) {
        //To keep sps/pps data buffer which is must needed for new mediacodec instance
        //Just to avaoid the resetting of sps/pps data
        next_bitstream_buffer_id_ -= 1;
      }
#endif
      Reset();
#if defined(ENABLE_WEBRTC_H264_CODEC)
      if (codec_info->codecType == webrtc::kVideoCodecH264) {
        //Don't drop this sps/pps data.
        next_bitstream_buffer_id_ += 1;
      }
#endif
    }
