...
public abstract class SelectionHandleController implements CursorController {
    ...
    protected static final int TEXT_DIRECTION_RTL = 2;

    /** The cursor controller images, lazily created when shown. */
    protected HandleView mStartHandle, mEndHandle;

    /** Whether handles should show automatically when text is selected. */
    protected boolean mAllowAutomaticShowing = true;
    ...
    protected int mFixedHandleX;
    protected int mFixedHandleY;
    ...
    protected void createHandlesIfNeeded(int startDir, int endDir) {
    ...
    protected void showHandlesIfNeeded() {
        if (!mIsShowing) {
            mIsShowing = true;
            mStartHandle.show();
            mEndHandle.show();
