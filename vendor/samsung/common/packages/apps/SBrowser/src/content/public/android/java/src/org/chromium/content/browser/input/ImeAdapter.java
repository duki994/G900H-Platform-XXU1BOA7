...
public class ImeAdapter {
    ...
    void updateImeAdapterSelection(int start, int end) {
        mInitialSelectionStart = start;
        mInitialSelectionEnd =  end;
    }
    ...
    boolean checkCompositionQueueAndCallNative(String text, int newCursorPosition,
        ...
        if (keyCode != COMPOSITION_KEY_CODE) {
            ...
            if (isCommit) {
                ...
                if(shouldEnableToDeferLoading()){
                    deferringEventRequired(text);
                }
                if(isFinish){
                    enableDefersLoadingFlag(true);
                }
