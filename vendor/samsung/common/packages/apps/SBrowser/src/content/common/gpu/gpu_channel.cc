...
int GpuChannel::GetNativeBitmap(BitmapParams imageParams,
        int32 surface_id, void** buffer)
{
  int ret = -1;
  DCHECK(surface_id);
  ...
  if (ret == -1) {
    LOG(WARNING)<<"GetBitmap"<<__FUNCTION__
           <<"can't find stub for surface_id: "<< surface_id;
  }
  return ret;
}
#endif
