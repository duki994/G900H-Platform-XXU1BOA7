...
scoped_ptr<RenderWidgetCompositor> RenderWidgetCompositor::Create(
  ...
  settings.impl_side_painting = cc::switches::IsImplSidePaintingEnabled();
    ...
    //Enabling kEnableGpuRasterization by default if no flags are defined.
    if(!settings.gpu_rasterization_enabled && !settings.gpu_rasterization_forced){
  ...
  settings.solid_color_scrollbar_color =
      cmd->HasSwitch(switches::kHideScrollbars)
          ? SK_ColorTRANSPARENT
          : SkColorSetARGB(255, 154, 156, 158);
#else
  settings.solid_color_scrollbar_color =
      cmd->HasSwitch(switches::kHideScrollbars)
          ? SK_ColorTRANSPARENT
...
void RenderWidgetCompositor::setContentTopOffset(float content_top_offset) {
	widget_->webwidget()->setContentTopOffset(content_top_offset);
}
#endif
