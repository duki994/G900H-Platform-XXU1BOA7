...
#if defined(OS_ANDROID)
static bool IsLowResolutionDevice() {
  gfx::DeviceDisplayInfo info;
  return info.GetDisplayWidth() * info.GetDisplayHeight() <= 480 * 800;
}
#endif
      ...
      disable_schedule_manage_(false)
{
  ...
  if (IsLowResolutionDevice()) {
    bytes_unmanaged_limit_step_ = 3 * 1024* 1024;
    bytes_default_per_client_   = 3 * 512 * 1024;
    bytes_minimum_per_client_   = 3 * 512 * 1024;
  }
...
#if defined(OS_ANDROID)
  uint64 mBytes = IsLowResolutionDevice() ? 3 : 16;
...
#if defined(OS_ANDROID)
  uint64 mBytes = IsLowResolutionDevice() ? 48 : 384;//256;
  return mBytes * 1024 * 1024;
#else
  return 1024 * 1024 * 1024;
#endif
...
void GpuMemoryManager::AssignSurfacesAllocations() {
  ...
  for (ClientStateList::const_iterator it = clients.begin();
    ...
    if (base::android::SysUtils::IsLowEndDevice())
      allocation.priority_cutoff_when_visible = MemoryAllocation::CUTOFF_ALLOW_NICE_TO_HAVE;
#endif
