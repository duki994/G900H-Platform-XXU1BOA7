...
bool RendererMediaPlayerManager::OnMessageReceived(const IPC::Message& msg) {
  ...
  IPC_BEGIN_MESSAGE_MAP(RendererMediaPlayerManager, msg)
    ...
    IPC_MESSAGE_HANDLER(MediaPlayerMsg_SetCCVisibility, OnSetCCVisibility)
#endif
...
void RendererMediaPlayerManager::OnTimeUpdate(int player_id,
  ...
  if (player){
    ...
    if(player->isMediaSourceType() && fullscreen_frame_ != NULL){
        Send(new MediaPlayerHostMsg_UpdateMediaSourceBuffering(routing_id(), player_id, player->bufferingPercentage()));
    }
#endif
  }
}
...
#if defined(S_MEDIAPLAYER_FULLSCREEN_CLOSEDCAPTION_SUPPORT)
void RendererMediaPlayerManager::UpdateCCVisibility(int player_id, int status){
    Send(new MediaPlayerHostMsg_UpdateCCVisibility(routing_id(), player_id, status));
}

void RendererMediaPlayerManager::OnSetCCVisibility(int player_id, bool visible) {
  WebMediaPlayerAndroid* player = GetMediaPlayer(player_id);
  if (player)
    player->OnSetCCVisibility(visible);
}
#endif
