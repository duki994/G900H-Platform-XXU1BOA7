...
class CONTENT_EXPORT BrowserMediaPlayerManager
 ...
 public:
  ...
  virtual void ShowAudioFocusFailMessage() OVERRIDE;
#endif

#if defined(S_MEDIAPLAYER_AUDIOFOCUS_GAIN_EVENT_FIX)
  virtual void OnAudioFocusGain(int player_id) OVERRIDE;
#endif
  ...
  void OnUpdateCCVisibility(int player_id, int status);
  void SetFullscreenCCVisibility(bool visible);
#endif
  ...
  void OnMediaSourceBufferingUpdate(int player_id, int percentage);
#endif
 ...
 private:
  ...
  // fullscreen.
  #if defined(S_NATIVE_SUPPORT)
  scoped_ptr<SbrContentVideoView> video_view_;
  #else
