...
void TouchEventQueue::ProcessTouchAck(InputEventAckState ack_result,
  ...
  if( acked_event.type != WebInputEvent::TouchStart )	  
...
void TouchEventQueue::OnGestureScrollEvent(
  ...
  if (gesture_event.event.type != blink::WebInputEvent::GestureScrollBegin && gesture_event.event.type != blink::WebInputEvent::GesturePinchBegin)
    return;
  ...
  TRACE_EVENT1("input", "TouchEventQueue::OnGestureScrollEvent2. now touch will be DROP_TOUCHES_IN_SEQUENCE", "type: ", gesture_event.event.type);
  touch_filtering_state_ = DROP_TOUCHES_IN_SEQUENCE;
