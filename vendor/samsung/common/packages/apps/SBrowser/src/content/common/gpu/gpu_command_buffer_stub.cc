...
int GpuCommandBufferStub::GetNativeBitmap(BitmapParams imageParams, void** buffer)
{
  ...
  if(!(decoder_ && MakeCurrent())) {
     LOG(ERROR)<<"GetBitmap"<<__FUNCTION__
            <<"MakeCurrent failed"<<"decoder "<<decoder_;
     return -1;
  }
  if(!(glGetError() == GL_NO_ERROR)) {
      LOG(ERROR)<<"GetBitmap"<<__FUNCTION__
             <<"decoder makecurrent FAILED **************"<<glGetError();
  }
  ...
  glReadPixels(imageParams.xPos, imageParams.yPos,
        imageParams.width, imageParams.height, format, type, *buffer);
  if((error = glGetError()) == GL_NO_ERROR) {
      FlipImageVertically(imageParams.width,
          imageParams.height, *buffer, getSize(imageParams.imageFormat));
  } else
     LOG(ERROR)<<"GetBitmap"<<__FUNCTION__
             <<"glReadPixels FAILED **************"<<error;
