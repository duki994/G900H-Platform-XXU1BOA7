...
#if defined(S_ENABLE_QCOM_BLACK_BOX)
#include <sys/stat.h>
#endif
...
#if defined(S_ENABLE_QCOM_BLACK_BOX)
  // create the v8-black-box directory
  // TODO: use application data dir instead of hardcoding
  const char* black_box_dir = "/data/data/org.chromium.chrome.testshell/v8-black-box";
  struct stat st;
  if (stat(black_box_dir, &st) == -1) {
    if (mkdir(black_box_dir, 0777) != 0) {
      LOG(ERROR) << "v8 blackbox dir creation failed";
    } else {
      if (chmod(black_box_dir, 0777) != 0) {
        LOG(ERROR) << "v8 blackbox dir permission failed";
      } else {
        LOG(INFO) << "v8 blackbox dir" + std::string(black_box_dir) + " created";
      }
    }
  }
#endif
