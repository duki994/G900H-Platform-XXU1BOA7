...
class PushRegistrationDispatcher;
#endif
...
class CONTENT_EXPORT RenderViewImpl
 ...
 public:
  ...
  virtual bool setBingAsCurrentSearchDefault() OVERRIDE;
  virtual bool isBingCurrentSearchDefault() OVERRIDE;
...
,bool isLastPointBounds = false
  ...
  // Method to close popup when select doesn't have items.
  void messageToClosePopup();
  int advancedIMEOptions() const { return advanced_ime_options_; }
  ...
  virtual void openUrlInNewTab(const base::string16& mouseClickUrl) OVERRIDE;
  ...
  virtual void OnLoadDataWithBaseUrl(const std::string& data,
  ...
  virtual void autoLoginFailure() OVERRIDE;
  #endif

  #if defined(S_FP_NEW_TAB_FIX)
  void checkforRPPBeforeTabClose();
  #endif

#if defined(S_INTUITIVE_HOVER)
  virtual void hoverHitTestResult(int contentType) OVERRIDE;
#endif
#if defined(S_PLM_P140721_03683)
 void OnEnterKeyOnFocused();
#endif
 protected:
 ...
 private:
  ...
  void OnDirectPaste(const base::string16& text);
  void OnRedo();
  ...
  void OnReleaseSnapshotDIB(TransportDIB::Handle dib_handle);
  ...
  void OnHandleSelectionDrop(int x, int y, const base::string16& text);
  void OnHandleSelectionDropOnFocusedInput(const base::string16& text, int dropAction);
  void OnGetFocusedInputInfo();
  
  void OnGetSelectionVisibilityStatus();
  ...
  void OnGetTouchedFixedElementHeight(int x, int y);
#if defined(SBROWSER_MULTI_SELECTION)
  void OnGetSelectionMarkupWithBounds(); // MULTI-SELECTION
#endif  
#if defined(OS_ANDROID)
  ...
  bool PaintContentRectToBitmap(const gfx::Rect& src_subrect,
                                float page_scale_factor);
  SkCanvas* CreatePlatformRGB565Canvas(int width,
                                        int height,
                                        bool is_opaque,
                                        uint8_t* data);
  static skia::BitmapPlatformDevice* CreateRGB565Device(int width, int height, bool is_opaque,
                                      uint8_t* data);

  bool IsValidBitmap(const SkBitmap& bitmap,int width,int height );

  void OnMoveNextTextOrSelectElement();
  void OnMovePreviousTextOrSelectElement();
  ...
  #if defined(S_MULTISELECTION_BOUNDS)
  void SyncSelectionIfRequired(bool isFlag = false);
  #else
  void SyncSelectionIfRequired();
  #endif

  // Add draw suspend
  void enableDrawSuspend();
  void disableDrawSuspend();
  ...
  // Tells whether the node is select input or not.
  bool IsSelectNode(const blink::WebNode& node) const;
  ...
  void OnHoverHighlight(const blink::WebInputEvent* hover_event, bool high_light);
  ...
  // Timer used to suspend rendering while page is loading
  base::OneShotTimer<RenderViewImpl> draw_suspend_timer;

  // Indicates if drawing is suspended
  bool is_draw_suspended_;
...
#if defined(ENABLE_PUSH_API)
  // The push registration dispatcher attached to this view, lazily initialized.
  PushRegistrationDispatcher* push_registration_dispatcher_;
#endif
  ...
  // Number of seconds to suspend draw when page load starts
  base::TimeDelta page_load_draw_suspend_time_;
