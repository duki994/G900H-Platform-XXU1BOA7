...
static struct sigaction action, oldact;
static int sig_init=0; 

static void sig_handler(int sig, siginfo_t *siginfo, void *ptr){
//Empty handler as we do not have any thing to do
// If you have the necessity to kill the process you could add your code here
} 
...
size_t SkMemoryStream::read(void* buffer, size_t size) {
    ...
    if(sig_init == 0) {
        ...
        //action.sa_flags = SA_RESETHAND | SA_RESTART | SA_SIGINFO;
