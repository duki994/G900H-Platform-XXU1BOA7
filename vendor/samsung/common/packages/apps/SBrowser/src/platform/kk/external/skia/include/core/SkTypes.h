...
public:
    ...
    void* reset(size_t size, OnShrink shrink = kAlloc_OnShrink,  bool* didChangeAlloc = NULL) {
     ...
     /**
     *  Reallocates the block to a new size. The ptr may or may not change.
     *  the reset_nothrow should be called with care as the method does not
     *  throw error upon allocation failure.
     */
    void* reset_nothrow(size_t size, OnShrink shrink = kAlloc_OnShrink,  bool* didChangeAlloc = NULL) {
        if (size == fSize || (kReuse_OnShrink == shrink && size < fSize)) {
            if (NULL != didChangeAlloc) {
                *didChangeAlloc = false;
            }
        ...
        sk_free(fPtr);
        /* Passing 0 in the 2nd argument ensures, abort is not invoked upon memory allocation failure*/
        fPtr = size ? sk_malloc_flags(size, 0) : NULL;
        fSize = 0;
        if ( fPtr )
             fSize = size;
