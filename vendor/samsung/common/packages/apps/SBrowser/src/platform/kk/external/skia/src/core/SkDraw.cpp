...
static void CallBitmapXferProc(const SkBitmap& bitmap, const SkIRect& rect,
    ...
    SkASSERT(pixels);
    /*
    Macro SkASSERT is disabled in Skia Android Builds.
    So adding Pointer validation checks for "pixels".
    As NULL pointer access is reported for variable "pixels"
    */
    if(NULL == pixels) {
        return;
    }
...
//{ Added by DMC.Graphics lab for Font Effect
#ifdef USE_SAMSUNG_FONT_EFFECT
static void initSFEffect(SFontEffect::SFEffect* effect, const SkPaint &paint, SkBitmap &bitmap,
        SFontEffect::SFBitmap &sfbitmap, int w, int h){
    effect->addText(paint.getColor(), effect->getTextOpacity());

    int paddedWidth = w + effect->getLeftOffset() + effect->getRightOffset();
    int paddedHeight = abs(h) + effect->getTopOffset() + effect->getBottomOffset();

    bitmap.setConfig(SkBitmap::kARGB_8888_Config, paddedWidth, paddedHeight);
    bitmap.allocPixels();
    bitmap.eraseColor(0);
    bitmap.setAlphaType(kPremul_SkAlphaType);

    sfbitmap.width = paddedWidth;
    sfbitmap.height = paddedHeight;
    sfbitmap.buffer = (uint8_t*)bitmap.getPixels();
    sfbitmap.num_channels = 4;

    effect->initEffect(paddedWidth, paddedHeight);
    effect->setDstBitmap(&sfbitmap);
}

static bool updateSFGlyph(const SkGlyph &glyph, SkGlyphCache* cache, SFontEffect::SFGlyph &sfGlyph) {
    if(glyph.fWidth) {
    ...
    // glyph metric
    sfGlyph.mBitmapW = glyph.fWidth;
    sfGlyph.mBitmapH = glyph.fHeight;
    sfGlyph.mLeft = glyph.fLeft;
    sfGlyph.mTop = glyph.fTop;

    // glyph raw bitmap
    if(sfGlyph.mBitmap != NULL) {
        delete[] sfGlyph.mBitmap;
    }

    sfGlyph.mBitmap = new uint8_t[sfGlyph.mBitmapW*sfGlyph.mBitmapH];

    if(glyph.fMaskFormat == SkMask::kBW_Format) {
        int srcRowBytes = glyph.rowBytes();
        int dstRowBytes = sfGlyph.mBitmapW;

        uint8_t* src = reinterpret_cast<uint8_t*>(glyph.fImage);
        uint8_t* dst = reinterpret_cast<uint8_t*>(sfGlyph.mBitmap);
        ...
        for (int y = sfGlyph.mBitmapH - 1; y >= 0; --y) {
...
static bool renderSFEffect(int top, int left, SkGlyphCache* cache, const SkGlyph& glyph,
        const SkPaint& paint, SFontEffect::SFBitmap& sfEmojiBitmap) {

    SFontEffect::SFEffect* effect = (SFontEffect::SFEffect*)paint.getFontEffect();
    if(effect == NULL) {
        return false;
    }
    ...
    if(glyph.fMaskFormat == SkMask::kA8_Format) {
        SFontEffect::SFGlyph sfGlyph;
        updateSFGlyph(glyph, cache, sfGlyph);
        effect->renderEffectLayer(left, top, &sfGlyph);
    }
        ...
        uint32_t cacheWidth = glyph.fWidth * sfEmojiBitmap.num_channels;
        uint32_t startY = 0;
        uint32_t endY = startY + (glyph.fHeight * cacheWidth);
        const uint8_t* cacheBuffer = (uint8_t*)glyph.fImage;

        for (uint32_t cacheY = startY, bitmapY = top * sfEmojiBitmap.width * sfEmojiBitmap.num_channels; cacheY < endY;
            cacheY += cacheWidth, bitmapY += sfEmojiBitmap.width * sfEmojiBitmap.num_channels) {
                memcpy(&sfEmojiBitmap.buffer[bitmapY + left*sfEmojiBitmap.num_channels], &cacheBuffer[cacheY], cacheWidth);
        }
    }
...
#ifdef USE_SAMSUNG_FONT_EFFECT
static void measure_text(SkGlyphCache* cache, SkDrawCacheProc glyphCacheProc,
                const char text[], size_t byteLength, SkRect* bounds) {
    bounds->set(SK_ScalarMax, SK_ScalarMin, SK_ScalarMin, SK_ScalarMax);
    ...
    while (text < stop) {
        ...
        int32_t nPenX = SkFixedRound(x) + glyph.fLeft;
        int32_t nPenY = SkFixedRound(y) + glyph.fTop;


        if (bounds->fBottom > nPenY) {
            bounds->fBottom = nPenY;
        }
        if (bounds->fLeft > nPenX) {
            bounds->fLeft = nPenX;
        }
        if (bounds->fRight < nPenX + glyph.fWidth) {
            bounds->fRight = nPenX + glyph.fWidth;
        }
        if (bounds->fTop < nPenY + glyph.fHeight) {
            bounds->fTop = nPenY + glyph.fHeight;
        }
    }
...
#ifdef USE_SAMSUNG_FONT_EFFECT
    //{ Added by DMC.Graphics lab for Font Effect
    if(paint.hasTextEffect()) {
        drawSFEffect(text,byteLength, x, y, paint);
        return;
    }
    //}
#endif
...
//{ Added by DMC.Graphics lab for Font Effect
#ifdef USE_SAMSUNG_FONT_EFFECT
static void measure_pos_text(SkGlyphCache* cache, SkDrawCacheProc glyphCacheProc,
        const char text[], size_t byteLength, const SkScalar pos[], SkScalar constY,
        const SkMatrix* matrix, int scalarsPerPosition, AlignProc alignProc, SkIRect* bounds) {
    bounds->set(SK_FixedMax, SK_FixedMin, SK_FixedMin, SK_FixedMax);
    if(pos == NULL) {
        return;
    }
    ...
    while (text < stop) {
        ...
        int top = SkFixedFloor(fixedLoc.fY + SK_FixedHalf) + glyph.fTop - SkScalarRound(matrix->getTranslateY());
        int left = SkFixedFloor(fixedLoc.fX + SK_FixedHalf) + glyph.fLeft - SkScalarRound(matrix->getTranslateX());

        if (bounds->fBottom > top) {
            bounds->fBottom = top;
        }
        if (bounds->fLeft > left) {
            bounds->fLeft = left;
        }
        if (bounds->fRight < left + glyph.fWidth) {
            bounds->fRight = left + glyph.fWidth;
        }
        if (bounds->fTop < top + glyph.fHeight) {
            bounds->fTop = top + glyph.fHeight;
        }
...
void SkDraw::drawPosText(const char text[], size_t byteLength,
    ...
    //{ Added by DMC.Graphics lab
#ifdef USE_SAMSUNG_FONT_EFFECT
    if(paint.hasTextEffect()) {
        drawPosSFEffect(text,byteLength, pos, constY, scalarsPerPosition, paint);
        return;
    }
#endif
    //}
...
//{ Added by DMC.Graphics lab for Font Effect
#ifdef USE_SAMSUNG_FONT_EFFECT
void SkDraw::drawSFEffect(const char text[], size_t byteLength,
    SkScalar x, SkScalar y, const SkPaint& paint) const {
    SkASSERT(byteLength == 0 || text != NULL);
    ...
    SkAutoKern          autokern;
    SkDraw1Glyph        d1g;
    d1g.init(this, blitter, cache, paint);
    ...
    //{ Added by DMC.Graphics lab, Font Effect Core Logic
    SFontEffect::SFEffect* effect = (SFontEffect::SFEffect*)paint.getFontEffect();
    if(effect == NULL) {
        return;
    }
    ...
    SkBitmap bitmap;
    SFontEffect::SFBitmap sfbitmap;
    SkRect bounds;
    measure_text(cache, glyphCacheProc, text, byteLength, &bounds);
    initSFEffect(effect, paint, bitmap, sfbitmap, bounds.width(), bounds.height());

    // Emoji bitmap allocation
    SFontEffect::SFBitmap sfEmojiBitmap(sfbitmap.width, sfbitmap.height);
    bool hasEmoji = false;
    //}
    ...
    while (text < stop) {
        ...
        fx += autokern.adjust(glyph);
        int top  = SkFixedFloor(fy) + glyph.fTop;
        int left = SkFixedFloor(fx) + glyph.fLeft;

        hasEmoji = renderSFEffect(top, left, cache, glyph, paint, sfEmojiBitmap);
        fx += glyph.fAdvanceX;
        fy += glyph.fAdvanceY;
    }

    effect->doEffectPostProcessing();
    effect->compositeLayers(&sfbitmap);

    // copy color emoji
    if(hasEmoji == true) {
        SFontEffect::SFBlender::drawBitmapBlend32(0, 0, &sfbitmap, sfEmojiBitmap.buffer,
            sfEmojiBitmap.width, sfEmojiBitmap.height, sfEmojiBitmap.width*sfEmojiBitmap.num_channels,
            SFontEffect::SFBlender::SFE_BLEND_DST_OVER, SFontEffect::SFColorFormat::RGBA);
    }

    SkMatrix matrix_bitmap;
    matrix_bitmap.setTranslate(0 , 0);
    this->drawBitmap(bitmap,matrix_bitmap,paint);

    sfbitmap.buffer = NULL;
}

void SkDraw::drawSFEffectBitmap(const SkBitmap& bitmap, const SkMatrix& matrix,const SkPaint& origPaint) const
{
    SkDEBUGCODE(this->validate();)
...
void SkDraw::drawPosSFEffect(const char text[], size_t byteLength,
                             const SkScalar pos[], SkScalar constY,
                             int scalarsPerPosition, const SkPaint& paint) const {
    SkASSERT(byteLength == 0 || text != NULL);
    SkASSERT(1 == scalarsPerPosition || 2 == scalarsPerPosition);
    ...
    SFontEffect::SFBitmap sfbitmap;
    SkIRect bounds;
    measure_pos_text(cache, glyphCacheProc, text, byteLength, pos,
        constY, fMatrix, scalarsPerPosition, alignProc, &bounds);
    initSFEffect(effect, paint, bitmap, sfbitmap, bounds.width(), bounds.height());
    ...
    if (cache->isSubpixel()) {
        ...
        if (SkPaint::kLeft_Align == paint.getTextAlign()) {
            while (text < stop) {
                ...
                //{ Added by DMC.Graphics lab, Font Effect Core Logic
                int top  = SkFixedFloor(fy) + glyph.fTop -SkScalarRound(fMatrix->getTranslateY());
                int left = SkFixedFloor(fx) + glyph.fLeft -SkScalarRound(fMatrix->getTranslateX());
                int x = left - bounds.fLeft + effect->getLeftOffset();
                int y = top - bounds.fBottom + effect->getTopOffset();

                hasEmoji = renderSFEffect(y, x, cache, glyph, paint, sfEmojiBitmap);
                //}
        ...
        if (SkPaint::kLeft_Align == paint.getTextAlign()) {
            while (text < stop) {
                ...
                if (glyph.fWidth) {
                    ...
                    //{ Added by DMC.Graphics lab, Font Effect Core Logic
                    // Apply Local matix trnslate position to effect engine
                    int top  = SkFixedFloor(SkScalarToFixed(tms.fLoc.fY) + SK_FixedHalf) + glyph.fTop -SkScalarRound(fMatrix->getTranslateY());
                    int left = SkFixedFloor(SkScalarToFixed(tms.fLoc.fX) + SK_FixedHalf) + glyph.fLeft -SkScalarRound(fMatrix->getTranslateX());
                    int x = left - bounds.fLeft + effect->getLeftOffset();
                    int y = top - bounds.fBottom + effect->getTopOffset();
            ...
            while (text < stop) {
                ...
                if (glyph.fWidth) {
                    ...
                    //{ Added by DMC.Graphics lab, Font Effect Core Logic
                    // Apply Local matix trnslate position to effect engine
                    int top  = SkFixedFloor(fixedLoc.fY + SK_FixedHalf) + glyph.fTop -SkScalarRound(fMatrix->getTranslateY());
                    int left = SkFixedFloor(fixedLoc.fX + SK_FixedHalf) + glyph.fLeft -SkScalarRound(fMatrix->getTranslateX());
                    int x = left - bounds.fLeft + effect->getLeftOffset();
                    int y = top - bounds.fBottom + effect->getTopOffset();
                    hasEmoji = renderSFEffect(y, x, cache, glyph, paint, sfEmojiBitmap);
                }
    ...
    SkMatrix matrix_bitmap;
    matrix_bitmap.setIdentity();
    matrix_bitmap.setTranslate(SkIntToScalar(bounds.fLeft) - effect->getLeftOffset() + SkScalarRound(fMatrix->getTranslateX()),
        SkIntToScalar(bounds.fBottom) - effect->getTopOffset() + SkScalarRound(fMatrix->getTranslateY()));

    this->drawSFEffectBitmap(bitmap,matrix_bitmap, paint);
    sfbitmap.buffer = NULL;
}
#endif
//}
