...
#include "SkFontEffect.h"
#include <map>

typedef std::map<const SkPaint*, SkFontEffect*> SFontEffectMap;
static SFontEffectMap gfontEffectMap;
SK_DECLARE_STATIC_MUTEX(gMutex);

static SkFontEffect* _getFontEffect(const SkPaint* paint){
    SFontEffectMap::iterator pos = gfontEffectMap.find(paint);
    if(pos != gfontEffectMap.end()) {
        return pos->second;
    }
    return NULL;
}
...
/**
 * insert font effect instance into map. If already exist, then delete the previous font effect instance and update.
 *
 * @param paint     key value of map. SkPaint address
 * @param effect    effect instance for insert
 *
 * @return generated font effect instance
 */
static SkFontEffect* _insertFontEffect(const SkPaint* paint, SkFontEffect* effect) {
    SkAutoMutexAcquire am(gMutex);
    std::pair<SFontEffectMap::iterator, bool> ret = gfontEffectMap.insert(SFontEffectMap::value_type(paint, effect));

    // already exist
    if(ret.second == false) {
        // delete previous font effect instance
        SkFontEffect* unused = _getFontEffect(paint);
        if(unused != NULL) {
            delete unused;
            unused = NULL;
        }

        // update font effect instance
        gfontEffectMap[paint] = effect;
    }
...
/**
 * generate font effect instance and insert into map
 *
 * @param paint     key value of map. SkPaint address
 *
 * @return generated font effect instance
 */
static SkFontEffect* _generateFontEffect(const SkPaint* paint) {
    SkFontEffect* effect = new SkFontEffect();
    return _insertFontEffect(paint, effect);
}

/**
 * generate font effect instance which copied from source effect and insert into map
 *
 * @param paint     key value of map. SkPaint address
 * @param srcEffect source font effect
 *
 * @return generated font effect instance
 */
static SkFontEffect* _generateFontEffect(const SkPaint* paint, SkFontEffect* srcEffect) {
    SkFontEffect* effect = new SkFontEffect(*srcEffect);
    return _insertFontEffect(paint, effect);
}

static void _cleanFontEffect(SkPaint* paint) {
    if(gfontEffectMap.empty() == true) {
        return;
    }

    SkFontEffect* effect = _getFontEffect(paint);
    SkAutoMutexAcquire am(gMutex);
    if(effect != NULL) {
        delete effect;
        effect = NULL;
        gfontEffectMap.erase(paint);
    }
}
#endif
//}
...
SkPaint::SkPaint(const SkPaint& src) {
    ...
    //{ Added by DMC.Graphics lab for Font Effect
#ifdef USE_SAMSUNG_FONT_EFFECT
    SkFontEffect* srcFontEffect = src.getFontEffect();
    if(srcFontEffect != NULL) {
        _generateFontEffect(this, srcFontEffect);
    }
    else {
        _cleanFontEffect(this);
    }
#endif
    //}
}
...
SkPaint::~SkPaint() {
    ...
    //{ Added by DMC.Graphics lab for Font Effect
#ifdef USE_SAMSUNG_FONT_EFFECT
    SkFontEffect* fontEffect = getFontEffect();
    if(fontEffect != NULL) {
        _cleanFontEffect(this);
    }
#endif
...
#ifdef USE_SAMSUNG_FONT_EFFECT
/**
 * returning font effect instance
 *
 * @return font effect instance
 */
SkFontEffect* SkPaint::getFontEffect() const {
    return _getFontEffect(this);
}

/**
* Check is being used text effect
*
* @return bool                 True Effect applied, false Effect not applicable.
*/
bool SkPaint::hasTextEffect() const {
    SkFontEffect* fontEffect = getFontEffect();
    if(fontEffect == NULL) {
        return false;
    }
    return fontEffect->hasEffects();
}

int SkPaint::getTextEffectOffsetLeft() const {
    SkFontEffect* fontEffect = getFontEffect();
    if(fontEffect == NULL) {
        return 0;
    }
    return fontEffect->getLeftOffset();
}

int SkPaint::getTextEffectOffsetRight() const {
    SkFontEffect* fontEffect = getFontEffect();
    if(fontEffect == NULL) {
        return 0;
    }
    return fontEffect->getRightOffset();
}

int SkPaint::getTextEffectOffsetTop() const {
    SkFontEffect* fontEffect = getFontEffect();
    if(fontEffect == NULL) {
        return 0;
    }
    return fontEffect->getTopOffset();
}

int SkPaint::getTextEffectOffsetBottom() const {
    SkFontEffect* fontEffect = getFontEffect();
    if(fontEffect == NULL) {
        return 0;
    }
    return fontEffect->getBottomOffset();
}

/**
 * Adds the outer shadow text effect.
 *
 * @param angle    angle of outer shadow
 * @param offset   offset between outer shadow and text
 * @param softness softness of outer shadow
 * @param color    color of outer shadow. Support rgb color channel. Alpha value will be ignored.
 * @param blendingOpacity blending opacity of outer shadow. 0.0 means full transparent, 1.0 means full opacity.
 *
 * @return id of outer shadow. if -1, then fail to add outer shadow text effect.
 */
int SkPaint::addOuterShadowTextEffect(float angle, float offset, float softness, int color, float blendingOpacity) {
    GEN_ID_INC;
    SkFontEffect* fontEffect = getFontEffect();
    if(fontEffect == NULL) {
        fontEffect = _generateFontEffect(this);
    }
    return fontEffect->addOuterShadowText(angle, offset, softness, color, blendingOpacity);
}

/**
 * Adds the inner shadow text effect.
 *
 * @param angle    angle of inner shadow
 * @param offset   offset between inner shadow and text
 * @param softness softness of inner shadow
 * @param color    color of inner shadow. Support rgb color channel. Alpha value will be ignored.
 * @param blendingOpacity blending opacity of inner shadow. 0.0 means full transparent, 1.0 means full opacity.
 *
 * @return id of inner shadow. if -1, then fail to add inner shadow text effect.
 */
int SkPaint::addInnerShadowTextEffect(float angle, float offset, float softness, int color, float blendingOpacity) {
    GEN_ID_INC;
    SkFontEffect* fontEffect = getFontEffect();
    if(fontEffect == NULL) {
        fontEffect = _generateFontEffect(this);
    }
    return fontEffect->addInnerShadowText(angle, offset, softness, color, blendingOpacity);
}

/**
 * Adds the stroke text effect.
 *
 * @param size size of stroke
 * @param color color of stroke. Support rgb color channel. Alpha value will be ignored.
 * @param blendingOpacity blending opacity of stroke. 0.0 means full transparent, 1.0 means full opacity.
 * @param joinStyle
 * @param position position of stroke. Outside=0, Center=1, Inside=2.
 *
 * @return id of stroke. if -1, then fail to add stroke text effect.
 */
int SkPaint::addStrokeTextEffect(float size, int color, float blendingOpacity, int joinStyle, int position) {
    GEN_ID_INC;
    SkFontEffect* fontEffect = getFontEffect();
    if(fontEffect == NULL) {
        fontEffect = _generateFontEffect(this);
    }
    return fontEffect->addStrokeText(size, color, blendingOpacity, joinStyle, position);
}

/**
 * Adds the outer glow text effect.
 *
 * @param size size of outer glow
 * @param color color of outer glow. Support rgb color channel. Alpha value will be ignored.
 * @param blendingOpacity blending opacity of outer glow. 0.0 means full transparent, 1.0 means full opacity.
 * @param count number of gradient points
 * @param gradients gradient structures pointer. Refer to the definition of SFGradient.
 *
 * @return id of outer glow. if -1, then fail to add outer glow text effect.
 */
int SkPaint::addOuterGlowTextEffect(float size, int color, float blendingOpacity, int count, SkFontGradient* gradients) {
    GEN_ID_INC;
    SkFontEffect* fontEffect = getFontEffect();
    if(fontEffect == NULL) {
        fontEffect = _generateFontEffect(this);
    ...
    SFontEffect::SFGradient* sfgradients = new SFontEffect::SFGradient[count];
    for(int i = 0 ; i < count ; i++) {
        sfgradients[i].alpha = gradients[i].alpha;
        sfgradients[i].color = gradients[i].color;
        sfgradients[i].position = gradients[i].position;
    }
    int id = fontEffect->addOuterGlowText(size, color, blendingOpacity, count, sfgradients);
    delete[] sfgradients;
    return id;
}

/**
 * Adds the linear gradient text effect.
 *
 * @param angle angle of linear gradient
 * @param scale scale of linear gradient. The valid scale range is 0.0f to 1.5f.
 * @param blendingOpacity blending opacity of linear gradient. 0.0 means full transparent, 1.0 means full opacity.
 * @param count number of gradient points
 * @param gradients gradient structures pointer. Refer to the definition of SFGradient.
 *
 * @return id of linear gradient. if -1, then fail to add linear gradient text effect.
 */
int SkPaint::addLinearGradientTextEffect(float angle, float scale, float blendingOpacity, int count, SkFontGradient* gradients) {
    GEN_ID_INC;
    SkFontEffect* fontEffect = getFontEffect();
    if(fontEffect == NULL) {
        fontEffect = _generateFontEffect(this);
    ...
    int id = fontEffect->addLinearGradientText(angle, scale, blendingOpacity, count, sfgradients);
    delete[] sfgradients;
    return id;
}
...
/**
 * Removes the text effect.
 *
 * @param id The ID of the text effect to remove
 *
 */
void SkPaint::removeTextEffect(int id) {
    SkFontEffect* fontEffect = getFontEffect();
    if(fontEffect == NULL) {
        return;
    }
    GEN_ID_INC;
    fontEffect->removeEffect(id);
}

/**
 * Sets the text opacity for text effect.
 * Each text effects has a blendingOpacity value. This value uses for text and background blend if text effects set.
 *
 * @param blendingOpacity blending opacity of text for text effect. 0.0 means full transparent, 1.0 means full opacity.
 *
 */
void SkPaint::setTextEffectOpacity(float blendingOpacity){
    SkFontEffect* fontEffect = getFontEffect();
    if(fontEffect == NULL) {
        fontEffect = _generateFontEffect(this);
    }

    GEN_ID_INC_EVAL(blendingOpacity != fontEffect->getTextOpacity());
    fontEffect->setTextOpacity(blendingOpacity);
}

/**
 * Clears the text opacity for text effect.
 *
 */
void SkPaint::clearTextEffectOpacity(){
    SkFontEffect* fontEffect = getFontEffect();
    if(fontEffect == NULL) {
        return;
    }
    ...
    GEN_ID_INC;
    fontEffect->clearTextOpacity();
}

/**
 * Clears all of text effect.
 *
 */
void SkPaint::clearAllTextEffect() {
    SkFontEffect* fontEffect = getFontEffect();
    if(fontEffect == NULL) {
        return;
    }
