...
public:
    ...
    enum Format {
        ...
        kWBMP_Format,
        kWEBP_Format,
#if defined(PNG_TO_SPI)
        kSPI_Format,
        kLastKnownFormat = kSPI_Format,
#else
        kLastKnownFormat = kWEBP_Format,
#endif//#if defined(PNG_TO_SPI)
#if defined TEXTURE_COMPRESSION_SUPPORT
        // adding these behind last known format (no use for kLastKnownFormat)
        kETC1_Format,
        kFirstTCFormat = kETC1_Format,
        kETC2_Format,
        kASTC_Format,
        kPVRTC_Format,
        kIPT_Format,
        kLastTCFormat = kLastKnownFormat + 100, // since for now it is used in all TC decoders
#endif
    };
...
#if defined(PNG_TO_SPI)
DECLARE_DECODER_CREATOR(SPIImageDecoder);
#endif
