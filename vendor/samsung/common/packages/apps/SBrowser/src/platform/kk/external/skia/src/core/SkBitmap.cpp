...
int SkBitmap::ComputeBytesPerPixel(SkBitmap::Config config) {
    ...
    switch (config) {
        case kNo_Config:
#ifdef TEXTURE_COMPRESSION_SUPPORT
		case kPVRTC2_2:
#endif
...
// GRAPHICS_COMPRESSION_START
        case kPVRTC2_2:
        case kETC1_AlphaAtlas_Config:
        case kETC1_Config:
        case kETC2_Config:
        case kETC2_Alpha_Config:
        case kASTC:
...
static bool validate_alphaType(SkBitmap::Config config, SkAlphaType alphaType,
    ...
    switch (config) {
        ...
        case SkBitmap::kRGB_565_Config:
            ...
            break;
#ifdef TEXTURE_COMPRESSION_SUPPORT
        case SkBitmap::kETC1_AlphaAtlas_Config:
        case SkBitmap::kETC2_Alpha_Config:
        case SkBitmap::kASTC:
            alphaType = kPremul_SkAlphaType;
            break;
        case SkBitmap::kETC1_Config:
        case SkBitmap::kETC2_Config:
            alphaType = kOpaque_SkAlphaType;
            break;
#endif
        default:
...
#ifdef TEXTURE_COMPRESSION_SUPPORT
	if (rowBytes == 0 && (config < kPVRTC2_2 || config > kASTC)) { //GRAPHICS_COMPRESSION
#else
    if (rowBytes == 0) {
#endif
