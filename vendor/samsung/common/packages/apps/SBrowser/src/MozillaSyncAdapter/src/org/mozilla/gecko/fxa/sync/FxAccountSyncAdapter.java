...
public class FxAccountSyncAdapter extends AbstractThreadedSyncAdapter {
  ...
  public FxAccountSyncAdapter(Context context, boolean autoInitialize, Bundle extras) {
	  super(context, autoInitialize);
	  this.executor = Executors.newSingleThreadExecutor();
	  this.notificationManager = new FxAccountNotificationManager(NOTIFICATION_ID);
	  mContext = context;
    ...
    try {
      ...
      stateMachine.advance(state, StateLabel.Married, new LoginStateMachineDelegate() {
        ...
        public void handleFinal(State state) {
          ...
          try {
        	  if(errorBundle != null)
        		  notificationManager.update(context, fxAccount,errorBundle);
        	  else
        		  notificationManager.update(context, fxAccount,extras);
            if (state.getStateLabel() != StateLabel.Married) {
