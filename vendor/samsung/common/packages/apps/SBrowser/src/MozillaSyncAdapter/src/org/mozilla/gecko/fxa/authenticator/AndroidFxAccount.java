...
//      intent.putExtra("SIGN", 2);//If value is 1 signin. If value is 2 sign in fail. If value is 3 signed out.
//      intent.setAction("android.intent.action.MOZILLA_FIREFOX_ACCOUNT_SIGN_IN");
//      context.sendBroadcast(intent);          
      ...
      Intent intent = new Intent();
      intent.putExtra("SIGN", 1);//If value is 1 signin. If value is 2 sign in fail. If value is 3 signed out.
      intent.setAction("android.intent.action.MOZILLA_FIREFOX_ACCOUNT_SIGN_IN");
      context.sendBroadcast(intent);
...
//    fxAccount.clearSyncPrefs();
    fxAccount.enableSyncing();
		...
		Logger.info(LOG_TAG, "Failed to add account " + e.getMessage() + "!");
	}

    return fxAccount;
  }

  public void clearSyncPrefs() throws UnsupportedEncodingException, GeneralSecurityException, IllegalStateException {
