...
class Monsoon:
  ...
  def _ResetUSB():
    try:
      lsusb_line = subprocess.check_output('lsusb -d 04d8:fe23', shell=True)
    except subprocess.CalledProcessError:
      raise EnvironmentError('Cannot find Monsoon Power Monitor device')

    bus, dev = re.match('Bus (\d\d\d) Device (\d\d\d):', lsusb_line).groups()
    dev_node = "/dev/bus/usb/%s/%s" % (bus, dev)

    with open(dev_node, "w") as fd:
      import fcntl
