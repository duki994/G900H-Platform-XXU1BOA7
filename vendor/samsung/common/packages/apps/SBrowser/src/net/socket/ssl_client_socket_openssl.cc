...
int MapOpenSSLErrorSSL() {
  ...
  switch (ERR_GET_REASON(error_code)) {
    ...
    // openssl_workaround
#if defined(S_CLEAR_SESSION_CACHE_ON_VERSION_ERROR)
    case SSL_R_WRONG_VERSION_NUMBER:
      return ERR_SSL_PROTOCOL_VERSION_ERROR;
#endif
    default:
...
int SSLClientSocketOpenSSL::DoHandshake() {
  ...
  if (client_auth_cert_needed_) {
    ...
    if (ssl_error == SSL_ERROR_WANT_CHANNEL_ID_LOOKUP) {
      ...
      // openssl_workaround
      // sometimes, wrong ssl version is stored in ssl session cache.
      // clear session cache in case of ssl version error
#if defined(S_CLEAR_SESSION_CACHE_ON_VERSION_ERROR)
      if (net_error == ERR_SSL_PROTOCOL_VERSION_ERROR) {
        LOG(INFO) << "openssl workaround solution";
        SSLContext::GetInstance()->session_cache()->Flush();
