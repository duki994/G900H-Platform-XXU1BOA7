...
class NET_EXPORT HttpRequestHeaders {
 public:
  ...
  static HttpRequestHeaders getSbrExtraHeaders();
  static void addSbrExtraHeaders(std::string key, std::string value);
  static void removeSbrExtraHeader(std::string key);
  static void clearSbrExtraHeader();
  static void updateSbrExtraHeaders(std::string key, std::string value);
#endif
 private:
