...
static HttpRequestHeaders sbr_extra_headers;
#endif
...
#if defined(S_HTTP_REQUEST_HEADERS)
HttpRequestHeaders HttpRequestHeaders::getSbrExtraHeaders() {
  return sbr_extra_headers;
}

void HttpRequestHeaders::addSbrExtraHeaders(std::string key, std::string value) {
    sbr_extra_headers.SetHeaderIfMissing(key, value);
}

void HttpRequestHeaders::removeSbrExtraHeader(std::string key) {
    sbr_extra_headers.RemoveHeader(key);
}

void HttpRequestHeaders::clearSbrExtraHeader() {
    sbr_extra_headers.Clear();
}

void HttpRequestHeaders::updateSbrExtraHeaders(std::string key, std::string value) {
    //Check if it is AcceptLanguage header to set up q value
    if ( key == net::HttpRequestHeaders::kAcceptLanguage ) {
        sbr_extra_headers.SetHeader(key, net::HttpUtil::GenerateAcceptLanguageHeader(value));
    } else {
        sbr_extra_headers.SetHeader(key, value);
    }
}
#endif
}  // namespace net
