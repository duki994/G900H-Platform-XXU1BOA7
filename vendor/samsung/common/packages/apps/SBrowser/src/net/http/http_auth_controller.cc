    ...
    switch (result) {
      ...
      case HttpAuth::AUTHORIZATION_RESULT_REJECT:
        ...
        if (handler_->auth_scheme() == 3) {
          ...
          kerberos_auth_info_->challenger = HostPortPair::FromURL(auth_origin_);
          kerberos_auth_info_->scheme = HttpAuth::SchemeToString(handler_->auth_scheme());
          kerberos_auth_info_->realm = handler_->realm();
        }
#endif
        InvalidateCurrentHandler(INVALIDATE_HANDLER_AND_CACHED_CREDENTIALS);
        break;
...
int HttpAuthController::HandleAuthChallenge(
  ...
  return HandleAuthChallenge(headers, do_not_send_server_auth, establishing_tunnel, net_log);
}
#endif
...
#if defined(S_USE_SYSTEM_PROXY_AUTH_CREDENTIAL)
  auth_info_->did_use_http_auth = did_use_http_auth_;
#endif
}
