...
#if defined(SBROWSER_KERBEROS_FEATURE)
  base::FilePath path_1, path_2;
  /* Changes for data directory path */
  PathService::Get(base::DIR_ANDROID_APP_DATA, &path_1); 
  PathService::Get(base::DIR_ANDROID_APP_DATA, &path_2); 
  base::FilePath krb5_config_fp = path_1.Append(FILE_PATH_LITERAL("kerberos/krb5.conf"));
  base::FilePath krb5_ccname_fp = path_2.Append(FILE_PATH_LITERAL("kerberos/ccache/krb5cc")); 
  setenv("KRB5_CONFIG", krb5_config_fp.value().c_str(), 1); 
  setenv("KRB5CCNAME", krb5_ccname_fp.value().c_str() , 1); 
#endif  
  if (!initialized_)
    InitImpl();
  return initialized_;
