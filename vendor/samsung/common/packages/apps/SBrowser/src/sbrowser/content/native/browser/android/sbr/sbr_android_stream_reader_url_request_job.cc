...
#include "sbrowser/content/native/browser/android/sbr/sbr_android_stream_reader_url_request_job.h"

#include <string>
#include "sbrowser/content/native/browser/android/sbr/sbr_input_stream.h"
#include "sbrowser/content/native/browser/android/sbr/sbr_input_stream_reader.h"
...
using content::InputStream;
using content::InputStreamReader;
...
class InputStreamReaderWrapper :
 ...
 public:
  ...
  content::InputStream* input_stream() {
 ...
 private:
  ...
  scoped_ptr<content::InputStream> input_stream_;
  scoped_ptr<content::InputStreamReader> input_stream_reader_;
...
AndroidStreamReaderURLRequestJob::AndroidStreamReaderURLRequestJob(
    ...
    : URLRequestJob(request, network_delegate),
     ...
     weak_factory_(this) {
      LOG(INFO) << "SavePage:: scrap AndroidStreamReaderURLRequestJob";
  DCHECK(delegate_);
}
...
LOG(INFO)<<"SavePage :: AndroidStreamReaderURLRequestJob::GetMimeType :: ";
  bool ret =  delegate_->GetMimeType(
      env, request(), input_stream_reader_wrapper_->input_stream(), mime_type);

  if(0 != mime_type)
    LOG(INFO)<<"SavePage :: AndroidStreamReaderURLRequestJob::GetMimeType :: "<<*mime_type;
  return ret;
}
