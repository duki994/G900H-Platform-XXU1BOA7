...
void MediaSourcePlayer::OnMediaInterrupted() {
...
void MediaSourcePlayer::StartInternal() {
  ...
  if (!listener_.RequestAudioFocus()) {
    ...
    manager()->ShowAudioFocusFailMessage();
#else
    listener_.ShowAudioFocusRequestFailMessage();
#endif
    return;
  }
...
#if defined(S_NATIVE_SUPPORT) && !defined(S_UNITTEST_SUPPORT)
  if (!is_motion_listener_set_) {
    motion_listener_.CreateMediaPlayerMotionListener(
                                          base::android::GetApplicationContext());
    motion_listener_.RegisterReceiver();
    is_motion_listener_set_ = true;
  }
#endif
}
...
#if defined(S_MEDIAPLAYER_AUDIOFOCUS_GAIN_EVENT_FIX)
void MediaSourcePlayer::OnAudioFocusGain() {
  manager()->OnAudioFocusGain(player_id());
}
#endif
