...
void WebKitPlatformSupportImpl::SuspendSharedTimer() {
  ...
  LOG(INFO) << "WebKitPlatformSupportImpl::SuspendSharedTimer(): shared_timer_suspended_ = "<< shared_timer_suspended_;	
}
...
void WebKitPlatformSupportImpl::ResumeSharedTimer() {
  ...
  if (--shared_timer_suspended_ <= 0 &&
      (!shared_timer_.IsRunning() || shared_timer_fire_time_was_set_while_suspended_)) {
    
    if (shared_timer_suspended_ < 0){
        LOG(INFO) << "WebKitPlatformSupportImpl::ResumeSharedTimer(): shared_timer_suspended_["<< shared_timer_suspended_<<"] < 0, so reset to 0";
        shared_timer_suspended_ = 0;
    }
  ...
  LOG(INFO) << "WebKitPlatformSupportImpl::ResumeSharedTimer(): shared_timer_suspended_ = "<< shared_timer_suspended_;
}
