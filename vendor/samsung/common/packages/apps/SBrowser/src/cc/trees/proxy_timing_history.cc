...
const size_t kDurationHistorySize = 7;
const size_t kCommitDurationHistorySize = 30;
...
ProxyTimingHistory::ProxyTimingHistory()
    : draw_duration_history_(kDurationHistorySize),
      ...
      commit_to_activate_duration_history_(kCommitDurationHistorySize) {}

ProxyTimingHistory::~ProxyTimingHistory() {}

base::TimeDelta ProxyTimingHistory::DrawDurationEstimate() const {
  base::TimeDelta historical_estimate =
      draw_duration_history_.MovingAverage();
...
base::TimeDelta ProxyTimingHistory::BeginMainFrameToCommitDurationEstimate()
    const {
  return begin_main_frame_to_commit_duration_history_.MovingAverage();
}

base::TimeDelta ProxyTimingHistory::CommitToActivateDurationEstimate() const {
  return commit_to_activate_duration_history_.MovingAverage();
}
