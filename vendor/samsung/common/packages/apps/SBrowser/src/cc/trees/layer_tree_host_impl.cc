...
#if defined(SBROWSER_HIDE_URLBAR_UI_COMPOSITOR)
  if(output_surface_){
    output_surface_->OnScrollEnd(true);
  }  
#endif 
...
//Commenting the code for PLM: P140722-06648. 
 /* if (!has_scrollBar)
  ...
  if (did_overscroll && input_handler_client_) {
    ...
    params.causal_event_viewport_point = viewport_point;
...
#if defined(SBROWSER_HIDE_URLBAR_UI_COMPOSITOR)
  scroll_ignored_ = false;
  if(!did_scroll && !did_overscroll){
    scroll_ignored_ = true;
  }
#endif 
  return did_scroll;
}
...
// SAMSUNG CHANGE : Check for scrolling or fling
  ...
  if(output_surface_){
    ...
    if(top_controls_manager_)
      animation = top_controls_manager_->IsAnimationActive();
    if(scroll_ignored_ && !animation){
      output_surface_->OnScrollEnd(true);
    }
    else{
      output_surface_->OnScrollEnd(false);
    }
  }
  scroll_ignored_ = false;  
#endif 
...
void LayerTreeHostImpl::SetDeferCommitOnImpl(bool defer) {
    if(!pinch_gesture_active_ && !page_scale_animation_) 
