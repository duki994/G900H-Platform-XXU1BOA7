...
base::TimeDelta RollingTimeDeltaHistory::MovingAverage() const {
  if (sample_set_.size() == 0) return base::TimeDelta();
  base::TimeDelta avg = std::accumulate(
      sample_set_.begin(), sample_set_.end(), base::TimeDelta());
  return base::TimeDelta::FromMicroseconds(
      avg.InMicroseconds() / sample_set_.size());
}
