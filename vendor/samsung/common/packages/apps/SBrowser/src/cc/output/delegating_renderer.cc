...
class ResourceFence : public ResourceProvider::Fence {
	public:
		ResourceFence()
			: locked_(true)
		{}
...
//  LOG(INFO)<<"DelegatingRenderer::ReceiveSwapBuffersAck";
  ResourceProvider::Fence* fence = resource_fence_.get();
  if (fence) static_cast<ResourceFence*>(fence)->ReleaseLock();
  resource_fence_ = resource_provider_->GetReadLockFence();
  resource_provider_->SetReadLockFence(new ResourceFence());
