...
void OutputSurface::SetScrollType(int type) {
  client_->SetScrollType(type);
}
#endif

#if defined(SBROWSER_HIDE_URLBAR_HYBRID)
void OutputSurface::SetTopControlsHeight(int top_controls_height) {
  client_->SetTopControlsHeight(top_controls_height);
}
#endif
}  // namespace cc
