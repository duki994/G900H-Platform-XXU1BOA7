...
void ResourcePool::ReduceResourceUsage() {
  if (max_memory_usage_bytes_ == 0) {
   /*we want to free everything.  It's ok not to wait for busy resources because if the gpu is really using them,
     they will be orphaned after the browser tries to deleted them (i.e. the actual memory buffer will still be available
     until gpu is done with them).*/
