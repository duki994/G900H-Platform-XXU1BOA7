...
void DelegatedRendererLayer::SetBackgroundColor(SkColor color) {
  LayerTreeHost* layer_host = layer_tree_host();
  if (layer_host)
    layer_host->set_background_color(color);
}
#endif

bool DelegatedRendererLayer::CompositeAndReadback(void* pixels,
                                                  const gfx::Rect& rect) {
  LayerTreeHost* layer_host = layer_tree_host();
  if (!layer_host)
    return false;

  return layer_host->CompositeAndReadback(pixels, rect);
}
