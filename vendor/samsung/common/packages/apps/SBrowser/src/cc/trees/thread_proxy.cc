...
void ThreadProxy::SetContentTopOffsetToMainThread(float offset) {
  DCHECK(IsImplThread());
  Proxy::MainThreadTaskRunner()->PostTask(
      FROM_HERE,
      base::Bind(&ThreadProxy::SetContentTopOffset,
                  main_thread_weak_ptr_,
                  offset));
}

void ThreadProxy::SetContentTopOffset(float offset) {
  DCHECK(IsMainThread());
  if(layer_tree_host())
      layer_tree_host()->SetContentTopOffset(offset);
}
#endif
...
void ThreadProxy::StartScrollbarAnimationOnImplThread() {
  ...
  impl().layer_tree_host_impl->SetScrollbarRequestCount(false);
  if (impl().layer_tree_host_impl->GetScrollbarRequestCount()>0)
    return;
#endif
...
void ThreadProxy::DeferCommitsOnImplThread(bool active) {
  DCHECK(IsImplThread());
  Proxy::MainThreadTaskRunner()->PostTask(FROM_HERE,
...
void ThreadProxy::DeferCommitsOnGesture(bool active) {
  DCHECK(IsMainThread());
  layer_tree_host()->SetDeferCommits(active);
}
#endif
}  // namespace cc
