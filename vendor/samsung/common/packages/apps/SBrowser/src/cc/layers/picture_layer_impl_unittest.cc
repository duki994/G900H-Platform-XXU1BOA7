...
TEST_F(PictureLayerImplTest, ManageTilingsCreatesTilings) {
  ...
  pending_layer_->CalculateContentsScale(1.3f,  // ideal contents scale
                                         ...
                                         &result_scale_y,
                                         &result_bounds);
  ASSERT_EQ(host_impl_.settings().create_low_res_tiling ? 2u : 1u, pending_layer_->tilings()->num_tilings());
  EXPECT_FLOAT_EQ(
      1.3f,
  ...
  pending_layer_->CalculateContentsScale(1.8f,  // ideal contents scale
                                         ...
                                         &result_scale_y,
                                         &result_bounds);
  ASSERT_EQ(host_impl_.settings().create_low_res_tiling ? 4u : 2u, pending_layer_->tilings()->num_tilings());
  EXPECT_FLOAT_EQ(
      1.8f,
      pending_layer_->tilings()->tiling_at(0)->contents_scale());
  if( host_impl_.settings().create_low_res_tiling )	  
  ...
  pending_layer_->CalculateContentsScale(1.9f,  // ideal contents scale
                                         ...
                                         &result_scale_y,
                                         &result_bounds);
  ASSERT_EQ(host_impl_.settings().create_low_res_tiling ? 6u : 3u, pending_layer_->tilings()->num_tilings());
  EXPECT_FLOAT_EQ(
      1.9f,
      pending_layer_->tilings()->tiling_at(0)->contents_scale());
  if( host_impl_.settings().create_low_res_tiling )		  
...
TEST_F(PictureLayerImplTest, CreateTilingsEvenIfTwinHasNone) {
  ...
  pending_layer_->CalculateContentsScale(high_res_scale,
                                         ...
                                         &result_scale_y,
                                         &result_bounds);
  ASSERT_EQ(host_impl_.settings().create_low_res_tiling ? 2u : 1u, active_layer_->tilings()->num_tilings());
...
TEST_F(PictureLayerImplTest, PinchGestureTilings) {
  ...
  SetContentsScaleOnBothLayers(1.0f, 1.0f, 1.0f, false);
  float low_res_factor = host_impl_.settings().low_res_contents_scale_factor;
  EXPECT_EQ(host_impl_.settings().create_low_res_tiling ? 2u : 1u, active_layer_->tilings()->num_tilings());
  EXPECT_FLOAT_EQ(
      1.0f,
  ...
  EXPECT_EQ(host_impl_.settings().create_low_res_tiling ? 3u : 2u, active_layer_->tilings()->num_tilings());
  EXPECT_FLOAT_EQ(
      1.0f,
      active_layer_->tilings()->tiling_at(0)->contents_scale());
  EXPECT_FLOAT_EQ(
      host_impl_.settings().create_low_res_tiling? 0.5f : 0.22502249f,
      active_layer_->tilings()->tiling_at(1)->contents_scale());
  if ( host_impl_.settings().create_low_res_tiling )
    EXPECT_FLOAT_EQ(
        1.0f * low_res_factor,
  ...
  EXPECT_EQ(host_impl_.settings().create_low_res_tiling? 3u : 2u, active_layer_->tilings()->num_tilings());
  SetContentsScaleOnBothLayers(2.1f, 1.0f, 2.1f, false);
  EXPECT_EQ(host_impl_.settings().create_low_res_tiling? 4u : 2u, active_layer_->tilings()->num_tilings());
  if (host_impl_.settings().create_low_res_tiling)	
  EXPECT_FLOAT_EQ(
      2.0f,
      active_layer_->tilings()->tiling_at(0)->contents_scale());
...
TEST_F(PictureLayerImplTest, CleanUpTilings) {
  ...
  ASSERT_EQ(host_impl_.settings().create_low_res_tiling ? 4u : 2u, active_layer_->tilings()->num_tilings());
  EXPECT_FLOAT_EQ(
      1.f,
      active_layer_->tilings()->tiling_at(1)->contents_scale());
  if( host_impl_.settings().create_low_res_tiling)
  ...
  used_tilings.clear();
  active_layer_->CleanUpTilingsOnActiveLayer(used_tilings);
  ASSERT_EQ( host_impl_.settings().create_low_res_tiling ? 3u : 2u, active_layer_->tilings()->num_tilings());
...
TEST_F(PictureLayerImplTest, DontAddLowResDuringAnimation) {
  ...
  EXPECT_BOTH_EQ(num_tilings(), host_impl_.settings().create_low_res_tiling ? 2u : 1u);
  ...
  if (host_impl_.settings().create_low_res_tiling)
	EXPECT_BOTH_EQ(LowResTiling()->contents_scale(), low_res_factor);
  EXPECT_BOTH_EQ(num_tilings(), host_impl_.settings().create_low_res_tiling ? 3u : 2u);
  ...
  EXPECT_BOTH_EQ(num_tilings(), host_impl_.settings().create_low_res_tiling ? 4u : 2u);
}
...
TEST_F(PictureLayerImplTest, ReleaseResources) {
  ...
  pending_layer_->CalculateContentsScale(1.3f,  // ideal contents scale
                                         ...
                                         &result_scale_y,
                                         &result_bounds);
  EXPECT_EQ(host_impl_.settings().create_low_res_tiling ? 2u : 1u, pending_layer_->tilings()->num_tilings());
...
TEST_F(PictureLayerImplTest, NothingRequiredIfActiveMissingTiles) {
  ...
  EXPECT_EQ(active_layer_->tilings()->num_tilings(), host_impl_.settings().create_low_res_tiling ? 2u : 1u);
