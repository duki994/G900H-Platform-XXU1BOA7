...
// For calcurate proper max tile number for interest area under HD resolution 
const int kCalcurateMaxInterestArea = 21300;
...
size_t PictureLayerImpl::GetMaxTilesForInterestArea() const {
  if(layer_tree_impl()->settings().default_tile_size.width() < 512)
  {
    gfx::DeviceDisplayInfo info;
  	int display_width = info.GetPhysicalDisplayWidth();
  	int display_height = info.GetPhysicalDisplayHeight();
	  return (size_t) display_width * display_height / kCalcurateMaxInterestArea;
  }	

  if(layer_tree_impl()->min_page_scale_factor() < 1){
      return layer_tree_impl()->settings().max_tiles_for_interest_area / 2;
  }
...
bool PictureLayerImpl::IsRootLayerTiling() const {
  LayerImpl* layer_impl = FindFirstDrawContent(layer_tree_impl()->root_layer());

  if(layer_impl != NULL && layer_impl->id() == this->id()){
      return true;
  }
  return false;
}

LayerImpl* PictureLayerImpl::FindFirstDrawContent(LayerImpl* node_layer) const{
	for (OwnedLayerImplList::iterator it = node_layer->children().begin();
		 it != node_layer->children().end();
		 ++it){
		 	if((*it)->DrawsContent()){
				return (*it);
		 	}
			LayerImpl* layer_impl = FindFirstDrawContent(*it);
			if(layer_impl != NULL)
			{
			    return layer_impl;
			}
