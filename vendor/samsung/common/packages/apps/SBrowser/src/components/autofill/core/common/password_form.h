...
struct PasswordForm {
  ...
  // username input element or the Parent form.
  // True otherwise.
  ...
  // True if Password form isn't rendered. 
  bool is_hidden;
  #endif

  #if defined(S_FP_SIGNUP_POPUP_FIX)
  //For SignUp Pages, it is set to true
  //Assuming SignUp Page will have 2 Password fields, Unfortunately that's not true always.
  //Initial Approach to avoid Remember Password Popup for Sign Up Pages.
  bool is_signup_page;
  #endif
