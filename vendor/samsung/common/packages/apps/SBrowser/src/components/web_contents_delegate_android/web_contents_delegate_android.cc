...
void WebContentsDelegateAndroid::RendererUnresponsive(WebContents* source) {
  ...
  if (obj.is_null())
    return;
  LOG(INFO)<<"[WebContentsDelegateAndroid][RendererUnresponsive]";
...
void WebContentsDelegateAndroid::RendererResponsive(WebContents* source) {
  ...
  if (obj.is_null())
    return;
  LOG(INFO)<<"[WebContentsDelegateAndroid][RendererResponsive]";
...
//S_HTML5_CUSTOM_HANDLER_SUPPORT  START	 
 ...
 void WebContentsDelegateAndroid::RegisterProtocolHandler(WebContents* web_contents, const std::string& protocol, const GURL& url, const base::string16& title,
   bool user_gesture) {
    ...
    ScopedJavaLocalRef<jstring> java_protocol = ConvertUTF8ToJavaString(env, protocol);
    ScopedJavaLocalRef<jstring> java_url = ConvertUTF8ToJavaString(env, url.spec());
    ScopedJavaLocalRef<jstring> java_title = ConvertUTF16ToJavaString(env, title);
    Java_WebContentsDelegateAndroid_registerProtocolHandler(env, obj.obj(), java_protocol.obj(), java_url.obj(), java_title.obj(), user_gesture);
  }

  // Unregister the registered handler for URL requests with the given scheme.   
  void WebContentsDelegateAndroid::UnregisterProtocolHandler(WebContents* web_contents, const std::string& protocol, const GURL& url, bool user_gesture) {
    ...
    ScopedJavaLocalRef<jstring> java_protocol = ConvertUTF8ToJavaString(env, protocol);
    ScopedJavaLocalRef<jstring> java_url = ConvertUTF8ToJavaString(env, url.spec());
    Java_WebContentsDelegateAndroid_unregisterProtocolHandler( env, obj.obj(), java_protocol.obj(), java_url.obj(), user_gesture);
  }
