...
bool AutofillAgent::OnMessageReceived(const IPC::Message& message) {
  ...
  IPC_BEGIN_MESSAGE_MAP(AutofillAgent, message)
    ...
    IPC_MESSAGE_HANDLER(AutofillMsg_ShowAutofillPopupCalled, OnShowAutofillPopupCalled)
#endif
...
#if defined(S_HIDE_AUTOFILL_OPTIMIZATION)
  if(autofill_popup_is_showing_){
      HideAutofillUI();
      autofill_popup_is_showing_ = false;
  }
#else
  HideAutofillUI();
#endif
...
#if defined(S_HIDE_AUTOFILL_OPTIMIZATION)
void AutofillAgent::OnShowAutofillPopupCalled(bool showing) {
  autofill_popup_is_showing_ = showing;
}
#endif
