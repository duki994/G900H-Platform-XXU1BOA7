...
class Encryptor {
 public:
  ...
  //to convert utf16 to utf8 and call encrypt
  static bool EncryptString16_256(const base::string16& plaintext,
                                  std::string* ciphertext,
                                  std::string& raw_key);

  //to convert utf8 to utf16 and call encrypt
  static bool DecryptString16_256(const std::string& ciphertext,
                                  base::string16* plaintext,
                                  std::string& raw_key);

  // Encrypt a string using AES256.
  static bool EncryptString256(const std::string& plaintext,
                               std::string* ciphertext,
                               std::string& raw_key);

  // Decrypt a string using AES256.
  static bool DecryptString256(const std::string& ciphertext,
                               std::string* plaintext,
                               std::string& raw_key);
