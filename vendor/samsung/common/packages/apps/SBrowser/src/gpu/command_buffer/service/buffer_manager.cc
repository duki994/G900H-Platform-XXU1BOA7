...
void BufferManager::DoBufferData(
  ...
  scoped_ptr<int8, base::FreeDeleter> zero;
  if (!data) {
    int8* zeroes = static_cast<int8*>(calloc(size, sizeof(int8)));
    if (!zeroes) {
      // TODO(b.kelemen): this check handles allocation failure on Android.
      // It does not work on Linux where a failed allocation results in
      // immediate termination. See: http://crbug.com/73751
      ERRORSTATE_SET_GL_ERROR(
          error_state, GL_OUT_OF_MEMORY, "glBufferData", "out of memory");
      return;
    }
