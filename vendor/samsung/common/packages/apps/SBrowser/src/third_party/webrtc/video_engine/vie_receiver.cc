...
bool ViEReceiver::RegisterPayload(const VideoCodec& video_codec) {
  ...
  if (ModuleRTPUtility::StringCompare("H264", video_codec.plName, 4)) {
    rtp_receiver_->GetMediaReceiver()->RegisterH264FmtpParameters(
        video_codec.codecSpecific.H264.profile_level_id,
        video_codec.codecSpecific.H264.packetization_mode,
        video_codec.codecSpecific.H264.sprop_parameter_sets);
  }
#endif
