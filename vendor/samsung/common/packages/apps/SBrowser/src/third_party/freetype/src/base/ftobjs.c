...
#define FT_COMPONENT  trace_objs
                 ...
                 FT_Int32  load_flags )
  {
    ...
    {
        face->fetstate.ignore_hinting_instruction = SF_Ignore_HintingInstruction(face->size->metrics.y_ppem, face->family_name);
    }
#endif
...
// Added by DMC Graphis Lab for Samsung Font Engine Extensione
#ifdef FSN_FONT_SOLUTION    
    if ( face->fetstate.iHybridHintMode != SF_HYBRID_HINTING_NONE )
        SF_Apply_HybridHinting(face, glyph_index, &load_flags);
#endif
...
// Added by DMC Graphis Lab for Samsung Font Engine Extension
    ...
    if(face->fetstate.iGlyphWeightExtensionMode != SF_GLYPH_WEIGHT_NONE )
      SF_Apply_GlyphWeightExtension(face);
#endif
    return error;
  }
...
// Added by DMC Graphis Lab for Samsung Font Engine Extension
#ifdef FSN_FONT_SOLUTION
    if(face->fetstate.glyph_unicodes)
    {
        FT_FREE(face->fetstate.glyph_unicodes);
    }
#endif
...
// Added by DMC Graphis Lab for Samsung Font Engine Extension
    ...
    face->fetstate.fGlyphWeightGradientX = 0.f;
    face->fetstate.iGlyphWeightMinWeightY = 0;
    face->fetstate.iGlyphWeightMaxWeightY = 0;
    face->fetstate.fGlyphWeightGradientY = 0.f;
    ...
    if ( aface ) {
#ifdef FSN_FONT_SOLUTION
        if(FT_ALLOC(face->fetstate.glyph_unicodes, face->num_glyphs * sizeof( FT_ULong ))) {
            face->fetstate.glyph_unicodes = NULL;
            FT_TRACE4(( "Fail to alloc unicode data %d\n", error ));
        } else {
            FT_Error error       = FT_Err_Ok;
            FT_UInt glyphIndex;
            FT_ULong charcode;
            error = FT_Select_Charmap( face, FT_ENCODING_UNICODE );
            for(charcode = FT_Get_First_Char(face, &glyphIndex) ;
                glyphIndex != 0;
                charcode = FT_Get_Next_Char(face, charcode, &glyphIndex)) {
                if (charcode &&
                    (face->fetstate.glyph_unicodes[glyphIndex] == 0)) {
                    face->fetstate.glyph_unicodes[glyphIndex] = charcode;
                }
            }   
        }
#endif
