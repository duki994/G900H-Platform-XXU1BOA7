...
void Range::textBoundingBox(Vector<IntRect>& rects) const
{
    Node* startContainer = m_start.container();
    Node* endContainer = m_end.container();
    ...
    if (startContainer && endContainer) {
        ...
        for (Node* node = firstNode(); node != stopNode; node = NodeTraversal::next(*node)) {
            ...
            if (r && r->isText()) {
                ...
                int endOffset = (node == endContainer) ? m_end.offset() : numeric_limits<int>::max();
                RenderText* renderText = toRenderText(r);
                renderText->absoluteRectsForRange(rects, startOffset, endOffset, false, 0, r->isBR());
            }
        }
    }
}

void Range::textAndImagesBoundingBox(Vector<IntRect>& rects) const
{
    Node* startContainer = m_start.container();
    Node* endContainer = m_end.container();
    ...
    for (Node* node = firstNode(); node != stopNode; node = NodeTraversal::next(*node)) {
        ...
        if (renderer->isText()) {
            int startOffset = (node == startContainer) ? m_start.offset() : 0;
            int endOffset = (node == endContainer) ? m_end.offset() : numeric_limits<int>::max();
            toRenderText(renderer)->absoluteRectsForRange(rects, startOffset, endOffset, false, 0, renderer->isBR());
        } else if (renderer->isImage()) {
            rects.append(toRenderImage(renderer)->absoluteBoundingBoxRect());
        }
    }
}
