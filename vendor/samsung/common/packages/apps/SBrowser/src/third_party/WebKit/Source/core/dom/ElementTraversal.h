...
// Reverse Pre-order traversal skipping non-element nodes.
Element* previous(const Node&);
Element* previous(const ContainerNode&);
...
template <class NodeType>
inline Element* traversePreviousElementTemplate(NodeType& current)
{
...
inline Element* previous(const ContainerNode& current) { return traversePreviousElementTemplate(current); }
inline Element* previous(const Node& current) { return traversePreviousElementTemplate(current); }
