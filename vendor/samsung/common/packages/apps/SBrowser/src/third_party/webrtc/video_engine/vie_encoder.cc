...
bool ViEEncoder::Init() {
  ...
  if (vcm_.Codec(webrtc::kVideoCodecH264, &video_codec_h264) != VCM_OK) {
    WEBRTC_TRACE(webrtc::kTraceError, webrtc::kTraceVideo,
                    ViEId(engine_id_, channel_id_),
  ...
  WEBRTC_TRACE(webrtc::kTraceError, webrtc::kTraceVideo,
                    ViEId(engine_id_, channel_id_),
                    "ViEEncoder::%s Codec pass done", __FUNCTION__);

  send_padding_ = video_codec_h264.numberOfSimulcastStreams > 1;
  if (vcm_.RegisterSendCodec(&video_codec_h264, number_of_cores_,
  ...
  WEBRTC_TRACE(webrtc::kTraceError, webrtc::kTraceVideo,
                    ViEId(engine_id_, channel_id_),
                    "ViEEncoder::%s RegisterSendCodec done", __FUNCTION__);

  if (default_rtp_rtcp_->RegisterSendPayload(video_codec_h264) != 0) {
    WEBRTC_TRACE(webrtc::kTraceError, webrtc::kTraceVideo,
                    ViEId(engine_id_, channel_id_),
  ...
  WEBRTC_TRACE(webrtc::kTraceError, webrtc::kTraceVideo,
                    ViEId(engine_id_, channel_id_),
                    "ViEEncoder::%s RegisterSendPayload done for h264", __FUNCTION__);
#endif
  ...
  WEBRTC_TRACE(webrtc::kTraceError, webrtc::kTraceVideo,
                    ViEId(engine_id_, channel_id_),
                    "ViEEncoder::%s RegisterSendPayload done for VP8", __FUNCTION__);
#else
  VideoCodec video_codec;
