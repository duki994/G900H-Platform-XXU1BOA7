...
#if defined(S_ACCELERATED_ROTATE_TRANSFORM)
        // Layer should be accelerated if transform rotate repeatedly
        const int kRotateRepeatThreshold = 8;
        if (!m_hasAcceleratedRotate && renderer()->style()->transform().hasRotate() && ++m_countRotate > kRotateRepeatThreshold) {
            m_hasAcceleratedRotate = true;
#if defined(S_NOTIFY_ROTATE_STATUS)
            if (renderer()->frame() && renderer()->frame()->page()) {
                renderer()->frame()->page()->chrome().client().notifyRotateStatus();
            }
#endif 
        }
#endif     
