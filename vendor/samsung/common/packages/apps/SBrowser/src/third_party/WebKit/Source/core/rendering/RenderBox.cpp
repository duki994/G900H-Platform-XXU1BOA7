...
#if defined(S_SCROLL_EVENT)
void RenderBox::scrollRectToVisible(const LayoutRect& rect, const ScrollAlignment& alignX, const ScrollAlignment& alignY,bool value)
#else
...
#endif
{
    ...
    {	
    	if(frame() && frame()->page())
        	rect1=frame()->page()->chrome().ScreenwindowRect();
    }
#endif
    ...
    if (hasOverflowClip() && !restrictedByLineClamp) {
                ...
                if (frameElementAndViewPermitScroll(frameElementBase, frameView)) {
                    ...
                    if(value){               
                        if(!frame()->tree().parent()){
                            if(rect1.height()>rect1.width())//For portrait mode
                                viewRect.setHeight((rect1.height()/2.5)/frameView->visibleContentScaleFactor());
                            else if(rect1.height()<=rect1.width())//For landscape mode
                                viewRect.setHeight((rect1.height()/4.5)/frameView->visibleContentScaleFactor());
                        }
                    }
#endif
		...
		LayoutRect r = ScrollAlignment::getRectToExpose(viewRect, rect, alignX, alignY,value);
#else
            #if defined(S_IME_SCROLL_EVENT)
                if(frameView->getContentTopOffset() > 0 && viewRect.height().toInt() > frameView->getContentTopOffset()){
                    LayoutUnit contentTopOffset = LayoutUnit((int)(frameView->getContentTopOffset() / frameView->visibleContentScaleFactor()));
                    viewRect.setHeight(viewRect.height()-contentTopOffset);
                }
	     #endif
...
#if defined(S_SCROLL_EVENT)	
	if(value)
            parentBox->scrollRectToVisible(newRect, alignX, alignY,value);
	else
