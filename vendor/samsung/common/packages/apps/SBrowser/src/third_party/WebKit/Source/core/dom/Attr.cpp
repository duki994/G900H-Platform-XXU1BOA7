...
{
    bool sameValue = this->value() == value;
    if (m_element)
    ...
    if (m_element)
        m_element->didModifyAttribute(qualifiedName(), value, sameValue);
}
...
void Attr::childrenChanged(bool, Node*, Node*, int)
{
    ...
    if (m_element)
        m_element->attributeChanged(qualifiedName(), newValue, sameValue);
}
