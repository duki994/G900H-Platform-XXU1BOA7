...
void StyleResolver::adjustMajorContentWidthWithMediaQuery(int currentLayoutWidth, int& majorContentWidth)
{
    const int commonMediumForMediaQuery = 1024;
    int smallerWidth = currentLayoutWidth;
    int largerWidth = currentLayoutWidth;
    unsigned size = m_viewportDependentMediaQueryResults.size();
    for (unsigned i = 0; i < size; i++) {
        CSSPrimitiveValue* mediaQueryCssValue = static_cast<CSSPrimitiveValue*>(m_viewportDependentMediaQueryResults[i]->m_expression.value());
        int mediaQueryValue = -1;
        if (mediaQueryCssValue)
            mediaQueryValue = mediaQueryCssValue->getIntValue(); // Get each of media query value

        // Calculate AdjustedMajorContentWidth
        // 1) majorContentWidth < [ mediaQuery values to check ] < viewportWidth
        // 2) if there are mediaQuery values smaller than 1024, use largest one among them
        // 3) else, use smallest one
        if (mediaQueryValue < currentLayoutWidth && mediaQueryValue > majorContentWidth) {
            if (mediaQueryValue <= commonMediumForMediaQuery && (mediaQueryValue > smallerWidth || smallerWidth == currentLayoutWidth))
                smallerWidth = mediaQueryValue;
            if (mediaQueryValue > commonMediumForMediaQuery && mediaQueryValue < largerWidth)
                largerWidth = mediaQueryValue;
        }
    }
    if (smallerWidth < currentLayoutWidth || largerWidth < currentLayoutWidth) {
        if (smallerWidth < currentLayoutWidth)
            majorContentWidth = smallerWidth;
        else
            majorContentWidth = largerWidth;
        if (majorContentWidth%10 == 9 || majorContentWidth < commonMediumForMediaQuery)
            majorContentWidth++;
    }
}
#endif
