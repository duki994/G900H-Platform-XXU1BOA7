...
def _CommonChecks(input_api, output_api):
    ...
    # Here we use python code from chromium. Tecnically this is is a layering
    # violation. However, it doesn't hurt in practice and still feels better
    # than duplicating the common checks.
    original_path = list(sys.path)
    srcdir = input_api.PresubmitLocalPath()
    scriptdir = input_api.os_path.abspath(input_api.os_path.join(
        srcdir, '..', '..', 'tools', 'python', 'samsung'))
    sys.path.append(scriptdir)
    import internal_presubmit

    results = []
    results.extend(internal_presubmit.PanProjectChecks(
        input_api, output_api,
        excluded_paths=_EXCLUDED_PATHS, maxlen=800,
        license_header=license_header))
    results.extend(internal_presubmit.CheckDescription(input_api, output_api))
