...
inline uint32_t pack_config8888(uint32_t a, uint32_t r,
    ...
    return (a << (A_IDX << 3)) | (r << (R_IDX << 3)) |
           (g << (G_IDX << 3)) | (b << (B_IDX << 3));
#else
    return (a << ((3-A_IDX) << 3)) | (r << ((3-R_IDX) << 3)) |
           (g << ((3-G_IDX) << 3)) | (b << ((3-B_IDX) << 3));
#endif
}
...
inline void unpack_config8888(uint32_t color,
                              ...
                              uint32_t* g, uint32_t* b) {
#ifdef SK_CPU_LENDIAN
    *a = (color >> (A_IDX << 3)) & 0xff;
    *r = (color >> (R_IDX << 3)) & 0xff;
    *g = (color >> (G_IDX << 3)) & 0xff;
    *b = (color >> (B_IDX << 3)) & 0xff;
#else
    *a = (color >> ((3 - A_IDX) << 3)) & 0xff;
    *r = (color >> ((3 - R_IDX) << 3)) & 0xff;
    *g = (color >> ((3 - G_IDX) << 3)) & 0xff;
    *b = (color >> ((3 - B_IDX) << 3)) & 0xff;
#endif
}

#ifdef SK_CPU_LENDIAN
    static const int SK_NATIVE_A_IDX = SK_A32_SHIFT >> 3;
    static const int SK_NATIVE_R_IDX = SK_R32_SHIFT >> 3;
    static const int SK_NATIVE_G_IDX = SK_G32_SHIFT >> 3;
    static const int SK_NATIVE_B_IDX = SK_B32_SHIFT >> 3;
#else
    static const int SK_NATIVE_A_IDX = 3 - (SK_A32_SHIFT >> 3);
    static const int SK_NATIVE_R_IDX = 3 - (SK_R32_SHIFT >> 3);
    static const int SK_NATIVE_G_IDX = 3 - (SK_G32_SHIFT >> 3);
    static const int SK_NATIVE_B_IDX = 3 - (SK_B32_SHIFT >> 3);
#endif
...
inline void convert_config8888(uint32_t* dstPixels,
    ...
    for (int y = 0; y < height; ++y) {
        ...
        for (int x = 0; x < width; ++x) {
            *dstPixels++ = convert_pixel<DST_CFG, SRC_CFG>(*srcPixels++);
        }
        dstPix += dstRowBytes;
        srcPix += srcRowBytes;
    }
    ...
    if (srcConfig == dstConfig) {
        ...
        if (dstRowBytes == srcRowBytes &&
            ...
            for (int y = 0; y < height; ++y) {
                srcPixels = reinterpret_cast<const uint32_t*>(srcPix);
                dstPixels = reinterpret_cast<uint32_t*>(dstPix);
                memcpy(dstPixels, srcPixels, rowLen);
                srcPix += srcRowBytes;
                dstPix += dstRowBytes;
            }
            return;
