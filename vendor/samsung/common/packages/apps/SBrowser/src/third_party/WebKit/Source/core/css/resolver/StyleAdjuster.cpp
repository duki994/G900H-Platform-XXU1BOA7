...
#if 0//defined(S_NATIVE_SUPPORT)
    // TODO(jreguera) Force element to avoid main thread while compositing.
    // Set translateZ(0) for elements with fixed position and hidden overflow.
    if ((style->position() == FixedPosition) && (style->overflowX() == OHIDDEN) && (style->overflowX() == OHIDDEN)) {
        if (e && e->isHTMLElement()) {
            TransformOperations ops = style->transform();
            ops.operations().append (TranslateTransformOperation::create (
                    Length(0, WebCore::Fixed), Length(0, WebCore::Fixed), 0,
