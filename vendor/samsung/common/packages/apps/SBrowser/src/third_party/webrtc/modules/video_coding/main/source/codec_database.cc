...
bool VCMCodecDataBase::Codec(int list_id,
  ...
  switch (list_id) {
    ...
    case VCM_H264_IDX: {
      ...
      settings->qpMax = 56;
      settings->codecSpecific.H264.bSPS_PPS_Sent = true;
      settings->codecSpecific.H264.bFlagConfigFrameRead= true;

      strncpy(settings->codecSpecific.H264.profile_level_id, "42801E", 6);
      settings->codecSpecific.H264.packetization_mode = 1;
      // The sprop parameter values are for 480x680 size 
      // (since always we are getting the same size).
      // TODO: Update with a table for sprop parameters with different sizes.
      strncpy(settings->codecSpecific.H264.sprop_parameter_sets,
                 "Z0KAHtoHgUaAbQoTUA==,aM4G4g==", 29);
      return true;
    }
#endif
#ifdef VIDEOCODEC_VP8
...
VCMGenericEncoder* VCMCodecDataBase::CreateEncoder(
  ...
  switch (type) {
#if defined(VIDEOCODEC_H264) && defined(ENABLE_WEBRTC_H264_CODEC)
    case kVideoCodecH264:
