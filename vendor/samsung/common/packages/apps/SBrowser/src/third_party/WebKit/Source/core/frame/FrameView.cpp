...
#endif
#if ENABLE(FIT_TO_MAJOR)
    , m_majorContentsWidth(0)
    , m_majorContentsFitTrialCount(0)
#endif
{
    ASSERT(m_frame);
    init();
...
#if ENABLE(FIT_TO_MAJOR)
    if (m_frame->settings()->fitToMajorScreenWidth() && m_frame->isMainFrame() && rootForThisLayout == m_frame->document()->renderView() && m_isVisuallyNonEmpty && majorContentsWidth() >= 0) {
        if ((layoutSize().width() >= m_frame->settings()->fitToMajorScreenWidth() || (majorContentsWidth() && majorContentsWidth() < toRenderBox(rootForThisLayout)->pixelSnappedLayoutOverflowRect().width()))
            && m_frame->document()->viewportDescription().type != ViewportDescription::UserAgentStyleSheet) {
            if (fitToMajorContentsIfNeeded()) {
                rootForThisLayout->layout();
                int newContentsWidth = pixelSnappedIntRect(toRenderBox(rootForThisLayout)->visualOverflowRect()).width();
                if (layoutSize().width() < newContentsWidth) {
                    if (toRenderBox(rootForThisLayout)->pixelSnappedLayoutOverflowRect().x() < 0)
                        newContentsWidth += toRenderBox(rootForThisLayout)->pixelSnappedLayoutOverflowRect().x();
                    setMajorContentsWidth(newContentsWidth);
                }
            }
        }
    }
...
#if ENABLE(FIT_TO_MAJOR)
static int s_deviceWidth;
static int s_deviceHeight;
static int s_maxBoundToFit;
static int s_minBoundToFit;

const char* const shouldNotFitSiteList[] = {
    "http://www.oprah.com", // if click one menu a fixed element that covers all the screen is showe
    "http://issuu.com", // in the image slide contents, not to fit to contents is much better
    "http://taboola.com", // there was line break in menu items
    "http://www.globo.com", // contents overflow occurs again regardless of a viewport size
    "http://home.bt.com", // in some case, a specific element inserted twice
    "http://sport.bt.com", // in some case, a specific element inserted twice
    0
};

static void initializeFitToMajorParam(int width, int height)
{
    const int minBoundToFit = 640;
    s_deviceWidth = std::max(width, height);
    s_deviceHeight = std::min(width, height);
    s_maxBoundToFit = (s_deviceWidth >> 4) * 15;
    s_minBoundToFit = std::min(s_deviceWidth / 2, minBoundToFit);
}

static const RenderObject* findMajorBlock(const RenderObject* parent, int width, int height)
{
    const RenderObject* child = parent->firstChild();
    const RenderObject* major = 0;
    int majorWidth = 0;
    int majorHeight = 0;
    int minHeightBound = height / 2;
    while (child) {
        if (child->isRenderBlock()) {
            IntRect childRect = toRenderBlock(child)->pixelSnappedLayoutOverflowRect();
            if (childRect.x() < 0 || childRect.y() < 0) {
                IntRect childVisualRect = pixelSnappedIntRect(toRenderBox(child)->visualOverflowRect());
                childRect.setWidth(max(childRect.width() + childRect.x(), childVisualRect.width()));
                childRect.setHeight(max(childRect.height() + childRect.y(), childVisualRect.height()));
            }
            if (childRect.width() >= s_minBoundToFit && (childRect.height() >= minHeightBound || !childRect.height())) {
                if (childRect.width() > s_maxBoundToFit - 100 || (!childRect.height() && child->firstChild())) {
                    const RenderObject* leaf = findMajorBlock(child, width, height);
                    if (leaf)
                        return leaf;
                    if (childRect.height() >= minHeightBound && (!major || (majorWidth <= childRect.width() && majorHeight <= childRect.height())))  {
                        major = child;
                        majorWidth = childRect.width();
                        majorHeight = childRect.height();
                    }
                } else if (childRect.height() >= minHeightBound && (!major || (majorWidth <= childRect.width() && majorHeight <= childRect.height()))) {
                    major = child;
                    majorWidth = childRect.width();
                    majorHeight = childRect.height();
                }
...
static int checkContentsOnSideOfMajorBlock(const RenderObject* parent, const RenderObject* major, int left, int right, int top, int bottom, int& lLeft, int& lRight, int& rLeft, int& rRight)
{
    const RenderObject* child = parent->firstChild();
    int result = 0;
    while (child) {
        if (child->isRenderBlock()) {
            IntRect childRect = child->absoluteBoundingBoxRect();
            if ((childRect.x() <= left || childRect.x() + childRect.width() >= right )
                && (!childRect.height()
                    || (childRect.y() >= top && childRect.y() <=bottom)
                    || (childRect.y() + childRect.height() >= top && childRect.y() + childRect.height() <= bottom )
                    || (childRect.y() <= top && childRect.y() + childRect.height() >=bottom))) {
                if (childRect.height()  > 10  && child->style() && (childRect.x() + childRect.width() < right || childRect.x() > left)) {
                    result++;

                    // if the object is on the left side, set the left & right edge of it to lLeft & lRight
                    if (childRect.x() < left) {
                        if (lLeft < 0 || lLeft > childRect.x())
                            lLeft = childRect.x() > 0 ? childRect.x() : 0;

                        if (childRect.x() + childRect.width() < left) {
                            if (lRight < childRect.x() + childRect.width())
                                lRight = childRect.x() + childRect.width();
                        } else {
                            lRight = left;
                        }
                    }
                    // if the object is on the right side, set the left & right edge of it to rLeft & rRight
                    if (childRect.x() + childRect.width() > right) {
                        if (rRight < childRect.x() + childRect.width())
                            rRight = childRect.x() + childRect.width();

                        if (childRect.x() > right) {
                            if (!rLeft || rLeft > childRect.x())
                                rLeft = childRect.x();
                        } else {
                            rLeft = right;
                        }
                    }
                } else if (child->firstChild() && child != major) {
                    result += checkContentsOnSideOfMajorBlock(child, major, left, right, top, bottom, lLeft, lRight, rLeft, rRight);
                }
            }
        }
        child = child->nextSibling();
...
static bool isShouldNotFitSite(String url)
{
    for (int i = 0;;i++) {
        if (!shouldNotFitSiteList[i])
            break;
        if (url.startsWith(shouldNotFitSiteList[i]))
            return true;
    }
    return false;
}

int findMajorContentsWidth(const RenderView* renderView, const RenderBlock* major)
{
    IntRect majorBoxRect = major->absoluteBoundingBoxRect();

    // If majorBlock's width is too small, check whether one of the parents block is more suitable as a majorBlock
    if (major->width() < 900 &&  major->parent()->isRenderBlock()) {
        for (RenderObject* parent = major->parent(); parent && parent->isRenderBlock(); parent = parent->parent()) {
            if (major->width() < toRenderBox(parent)->width() && toRenderBox(parent)->width() < s_maxBoundToFit) {
                majorBoxRect.setX(toRenderBox(parent)->x());
                majorBoxRect.setWidth(toRenderBox(parent)->width());
            }
        }
    }

    // If there is some contents on the left/right side of the majorBock, check the boundary of them to extend the majorContentsWidth
    int majorBoxLeft = majorBoxRect.x();
    int majorBoxRight = majorBoxRect.width() + majorBoxRect.x();
    int majorBoxTop = majorBoxRect.y();
    int majorBoxBottom = majorBoxRect.height() + majorBoxRect.y();
    int majorBoxExtend = 0;
    int lLeft = -1; // left boundary of left side contents
    int lRight = 0; // right boundary of right side contents
    int rLeft = 0; // left boundary of right side contents
    int rRight = 0; // right boundary of right side contents
    checkContentsOnSideOfMajorBlock(renderView, major, majorBoxLeft, majorBoxRight, majorBoxTop, majorBoxBottom, lLeft, lRight, rLeft, rRight);

    if (lLeft < 0)
        lLeft = 0;
    if (rRight > s_deviceWidth)
        rRight = s_deviceWidth;
    if (lRight > lLeft)
        majorBoxExtend += lRight - lLeft;
    if (rRight > rLeft)
        majorBoxExtend += rRight - rLeft;

    int majorContentsWidth = 0;
    if (majorBoxRight - majorBoxLeft + majorBoxExtend < s_maxBoundToFit) {
        if (majorBoxRight - majorBoxLeft + majorBoxExtend <  s_deviceHeight) {
            majorContentsWidth = s_deviceHeight;
            majorBoxExtend = s_deviceHeight - (majorBoxRight - majorBoxLeft);
        } else {
            majorBoxExtend = majorBoxExtend > 10 ? majorBoxExtend : 10;
            majorContentsWidth = (majorBoxRight - majorBoxLeft  < renderView->width() - majorBoxExtend) ?  majorBoxRight - majorBoxLeft + majorBoxExtend : majorBoxRight - majorBoxLeft;
        }
        return majorContentsWidth;
    }
    return -1; // should not be checked again!
}

void FrameView::setMajorContentsWidth(int width)
{
    if (width > s_maxBoundToFit) // if try to fit to size larger than max bound, initialize.
        width = -1;
    m_majorContentsWidth = width;
    ViewportDescription description = m_frame->document()->viewportDescription();
    description.additionalMaxWidth = width <= 0 ? ViewportDescription::ValueAuto : width;
    m_frame->document()->setViewportDescription(description);
}

bool FrameView::fitToMajorContentsIfNeeded()
{
    const RenderView *renderView = m_frame->document()->renderView();

    if (!s_deviceWidth) {
        blink::WebRect rect = m_frame->chromeClient().screenInfo().rect;
        initializeFitToMajorParam(rect.width, rect.height);
    }

    Length viewportMaxWidth = m_frame->document()->viewportDescription().maxWidth;
    if ((viewportMaxWidth.isFixed() && viewportMaxWidth.getFloatValue() >= s_deviceWidth)
        || (layoutSize().width() >= s_deviceWidth && (contentsWidth() > layoutSize().width() ||  renderView->pixelSnappedLayoutOverflowRect().width()  > layoutSize().width()))) {
        setMajorContentsWidth(-1);
        return false;
    }

    // If already set, check whether it could extend by overflow (if not, try to recalculate up to 5 times)
    if (majorContentsWidth() > 0) {
        int newContentsWidth = max(pixelSnappedIntRect(renderView->visualOverflowRect()).width(), renderView->pixelSnappedLayoutOverflowRect().width() -renderView->pixelSnappedLayoutOverflowRect().x());
        if (majorContentsWidth() < newContentsWidth && newContentsWidth <= s_maxBoundToFit) {
            setMajorContentsWidth(newContentsWidth);
            return false;
        }
    }

    // Find majorBlock to fit
    const RenderBlock* majorBlock =  toRenderBlock(findMajorBlock(renderView, contentsWidth(), contentsHeight()));

    // Only try to fit if the majorBlock is found, and it's size is reasonable
    if (majorBlock && majorBlock->x().toInt() + majorBlock->width().toInt() >= 0 && majorBlock->y().toInt() + majorBlock->height().toInt() >= 0 && (!contentsWidth() || majorBlock->width().toInt() < contentsWidth())) {
        int majorContentsWidth = 0;

        // If the site is included in blacklist or fit tried too many times, fit should not work.
        if (m_majorContentsFitTrialCount > 5 || isShouldNotFitSite(m_frame->document()->url().string()))
            majorContentsWidth = -1;
        else
            majorContentsWidth = findMajorContentsWidth(renderView, majorBlock);

        if (majorContentsWidth > 0)
            m_frame->document()->styleResolver()->adjustMajorContentWidthWithMediaQuery(layoutSize().width(), majorContentsWidth);

        // If there is a viewport constrained object that can cover all the screen after fit, fit should not work.
        if (majorContentsWidth > 0 && hasViewportConstrainedObjects()) {
            int layoutHeightAfterFit = layoutSize().height()  * ((double)majorContentsWidth / layoutSize().width());
            for (ViewportConstrainedObjectSet::const_iterator it = m_viewportConstrainedObjects->begin(), end = m_viewportConstrainedObjects->end(); it != end; ++it) {
                RenderObject* viewportConstrainedObject = *it;
                if (viewportConstrainedObject->isBox()) {
                    if (toRenderBox(viewportConstrainedObject)->height().toInt() > layoutHeightAfterFit  && viewportConstrainedObject->style()->height().type() == Fixed) {
                        majorContentsWidth = -1;
                        break;
                    }
                }
        ...
        // Increase fit trial count - to prevent to try fit again and again
        if (majorContentsWidth > 0)
            m_majorContentsFitTrialCount++;

        setMajorContentsWidth(majorContentsWidth);
        return majorContentsWidth > 0;
    }
    return false;
}
#endif
