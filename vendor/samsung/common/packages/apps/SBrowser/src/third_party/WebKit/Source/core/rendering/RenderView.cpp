...
void RenderView::repaintImageAnimationsIfNeeded()
{
    if (m_renderersWithOffscreenAnimation.isEmpty())
        return;

    Vector<RenderObject*> visibleList;
    IntRect visibleRect = document().view()->visibleContentRect();
    {
        HashSet<RenderObject*>::iterator end = m_renderersWithOffscreenAnimation.end();
        for (HashSet<RenderObject*>::iterator it = m_renderersWithOffscreenAnimation.begin(); it != end; ++it) {
            if (!visibleRect.intersects((*it)->absoluteBoundingBoxRect()))
                continue;
            (*it)->repaint();
            visibleList.append(*it);
        }
    }

    {
        Vector<RenderObject*>::iterator end = visibleList.end();
        for (Vector<RenderObject*>::iterator it = visibleList.end(); it != end; ++it)
            m_renderersWithOffscreenAnimation.remove(*it);
    }
}

void RenderView::addRendererWithOffscreenAnimations(RenderObject& object)
{
    m_renderersWithOffscreenAnimation.add(&object);
}

void RenderView::removeRenderer(RenderObject& object)
{
    m_renderersWithOffscreenAnimation.remove(&object);
}
