...
#if defined(ENABLE_WEBRTC_H264_CODEC)
  void RegisterH264FmtpParameters(const char* profileLevelId,
                                  int packetizationMode,
                                  const char* spropParameterSets) OVERRIDE;
#endif
...
#if defined(ENABLE_WEBRTC_H264_CODEC)
  int32_t packetizationMode_;
  int32_t width_;
  int32_t height_;
  const char* profileLevelId_;
  const char* spropParameterSets_;
