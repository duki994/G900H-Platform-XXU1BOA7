    ...
    , m_refCount(1)
{
    if (pseudoElementName.isNull()) {
        m_pseudoElementSpecifier = NOPSEUDO;
    } else {
...
PassRefPtr<CSSValue> CSSComputedStyleDeclaration::getPropertyCSSValue(CSSPropertyID propertyID, EUpdateLayout updateLayout) const
{
    ...
    if (updateLayout) {
        ...
        if (ancestor && !styledNode->isInShadowTree()
            && styledNode->isElementNode()
            && styledNode->parentRenderStyle()) {
            ...
            if (canDoStrictRecalc) {
                ...
                if (styledNode->needsStyleRecalc()
                    ...
                    && !styledElement->activeAnimations()) {
                    DocumentLifecycle::State oldState = document.lifecycle().state();
