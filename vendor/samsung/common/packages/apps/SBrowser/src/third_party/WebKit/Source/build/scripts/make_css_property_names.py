...
class CSSPropertiesWriter(in_generator.Writer):
    ...
    def make_serialized_property(self, property):
        if property['debug_only']:
            if not property['name'].startswith('-webkit-'):
                return '-webkit-' + property['name']
    ...
    def __init__(self, file_paths):
        ...
        for offset, property in enumerate(self._properties):
            ...
            if "Debug" not in sys.argv[4]:
                property['serialized_name'] = self.make_serialized_property(property)
            else:
                property['serialized_name'] = property['name']
    ...
    def generate_implementation(self):
        ...
        current_offset = 0
        if "Debug" not in sys.argv[4]:
          self._properties_for_build_type = filter(lambda property: not property['debug_only'], self._properties)
        else:
          self._properties_for_build_type = self._properties
        for property in self._properties:
            property_offsets.append(current_offset)
            current_offset += len(property["serialized_name"]) + 1

        gperf_input = GPERF_TEMPLATE % {
            'license': license.license_for_generated_cpp(),
            'class_name': self.class_name,
            'property_name_strings': '\n'.join(map(lambda property: '    "%(serialized_name)s\\0"' % property, self._properties)),
            'property_name_offsets': '\n'.join(map(lambda offset: '    %d,' % offset, property_offsets)),
            'property_to_enum_map': '\n'.join(map(lambda property: '%(name)s, %(enum_name)s' % property, self._properties_for_build_type + self._aliases)),
