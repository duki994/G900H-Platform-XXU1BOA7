...
bool RenderObject::scrollRectToVisible(const LayoutRect& rect, const ScrollAlignment& alignX, const ScrollAlignment& alignY,bool value)
#else
...
#if defined(S_SCROLL_EVENT)
    if(value)
    	enclosingBox->scrollRectToVisible(rect, alignX, alignY,value);
    else
...
bool RenderObject::willRenderImage(ImageResource*)
{
    ...
    if (!document().view()->isVisible())
        return false;

    IntRect visibleRect = document().view()->visibleContentRect();
    if (!visibleRect.intersects(absoluteBoundingBoxRect())) {
        view()->addRendererWithOffscreenAnimations(*this);
        return false;
    }
    return true;
}
