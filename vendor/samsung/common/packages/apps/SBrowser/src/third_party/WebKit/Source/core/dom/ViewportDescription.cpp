...
PageScaleConstraints ViewportDescription::resolve(const FloatSize& initialViewportSize) const
{
    ...
    if (resultMinWidth != ViewportDescription::ValueAuto || resultMaxWidth != ViewportDescription::ValueAuto) {
        ...
        if (minWidthEnabled && resultWidth == 1280 && resultWidth > resultMinWidth)
            resultWidth = resultMinWidth;
#endif
#if ENABLE(FIT_TO_MAJOR)
        if (additionalMaxWidth > 0 && resultWidth > additionalMaxWidth) // If there is additional max width to fit to major contents, constrain the width with it
            resultWidth = additionalMaxWidth;
#endif
    }
