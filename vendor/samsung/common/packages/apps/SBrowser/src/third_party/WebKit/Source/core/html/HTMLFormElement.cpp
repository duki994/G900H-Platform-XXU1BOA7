...
{
    LOG(INFO)<<"FP HTMLFormElement::rendererIsNeeded";
    if (!m_wasDemoted){
        bool renderIsNeeded = HTMLElement::rendererIsNeeded(style);
        #if defined(S_FP_HIDDEN_FORM_FIX)
        // Wait for Some time so that Form is Rendered.	 
        if(renderIsNeeded){
...
#if defined(S_FP_HIDDEN_FORM_FIX)
void HTMLFormElement::SubmitOnTimer(){
	Frame *frame = document().frame();
	// If form is already submitted there is no need to check form visibility and autofill.
	if(frame && !this->wasWebLoginSubmitted())
		frame->loader().client()->dispatchCheckFormVisibilityAndAutoFill();
}
#endif
...
{
	LOG(INFO)<<"FP: HTMLFormElement::submitImplicitly";
    ...
    for (unsigned i = 0; i < elements.size(); ++i) {
        ...
        if (!seenDefaultButton && control->canBeSuccessfulSubmitButton()) {
            ...
            if (control->isSuccessfulSubmitButton()) {
                if (control->renderer()) {
					LOG(INFO)<<"FP: HTMLFormElement::submitImplicitly dispatchSimulatedClick";
                    control->dispatchSimulatedClick(event);
                    return;
                }
            } else if (fromImplicitSubmissionTrigger) {
				LOG(INFO)<<"FP: HTMLFormElement::submitImplicitly Default (submit) button is not activated return";
    ...
    if (fromImplicitSubmissionTrigger && submissionTriggerCount == 1){
		LOG(INFO)<<"FP: HTMLFormElement::submitImplicitly prepareForSubmission";
...
{
	LOG(INFO)<<"FP: HTMLFormElement::prepareForSubmission";
    ...
    if (!validateInteractively(event)) {
        m_isSubmittingOrPreparingForSubmission = false;
		LOG(INFO)<<"FP: HTMLFormElement::prepareForSubmission return false";
        return false;
    }
...
{
	LOG(INFO)<<"FP: HTMLFormElement::submitFromJavaScript ";
...
{
    LOG(INFO)<<"FP: HTMLFormElement::scheduleFormSubmission";
    ...
    if (submission->action().isEmpty()){
	    LOG(INFO)<<"FP: HTMLFormElement::scheduleFormSubmission ACTION EMPTY return";
        return;
    	}
	...
	LOG(INFO)<<"FP: HTMLFormElement:: navigationScheduler scheduleFormSubmission .....";
...
bool HTMLFormElement::wasWebLoginSubmitted() const
{
    return m_wasWebLoginSubmitted;
}
