...
#if defined(S_PLM_P140703_05388)
#undef AUTOSIZING_CLUSTER_HASH
#else
#define AUTOSIZING_CLUSTER_HASH
#endif

#define TURN_OFF_EDITABLE_DIV_TEXT_AUTOSIZING
...
#ifdef TURN_OFF_EDITABLE_DIV_TEXT_AUTOSIZING
static bool isIFrameEditablePresent(const RenderObject * renderer)
{
    bool tagValueCheck = true;
    Element* ownerElement = renderer->document().ownerElement();
    if (ownerElement && ownerElement->hasTagName(iframeTag)) {
        if(renderer->document().body() && renderer->document().body()->isContentEditable())
            tagValueCheck = false;
    }
    return tagValueCheck;
}
#endif
...
float TextAutosizer::clusterMultiplier(WritingMode writingMode, const TextAutosizingWindowInfo& windowInfo, float textWidth) const
{
    ...
    float mul = m_document->settings()->accessibilityFontScaleFactor();
...
bool TextAutosizer::isAutosizingContainer(const RenderObject* renderer)
{
    ...
    // - Must not be editable container with iFrame - SBrowser
    Node* node = renderer->generatingNode();
#ifdef TURN_OFF_EDITABLE_DIV_TEXT_AUTOSIZING
        bool tagValueCheck = isIFrameEditablePresent(renderer);
        if ( (node && !node->hasChildNodes())
            || (node && node->isContentEditable() && !tagValueCheck)
...
bool TextAutosizer::isIndependentDescendant(const RenderBlock* renderer)
{
    ...
    RenderBlock* containingBlock = renderer->containingBlock();
        ...
        || renderer->isTextArea()
        || (renderer->style()->userModify() != READ_ONLY && tagValueCheck);
#else
    return renderer->isRenderView()
        || renderer->isFloating()
        || renderer->isOutOfFlowPositioned()
...
bool TextAutosizer::compositeClusterShouldBeAutosized(Vector<TextAutosizingClusterInfo>& clusterInfos, float blockWidth)
{
    ...
    for (size_t i = 0; i < clusterInfos.size(); ++i) {
#ifdef TURN_OFF_EDITABLE_DIV_TEXT_AUTOSIZING
        bool tagValueCheck = isIFrameEditablePresent(clusterInfos[i].root);
