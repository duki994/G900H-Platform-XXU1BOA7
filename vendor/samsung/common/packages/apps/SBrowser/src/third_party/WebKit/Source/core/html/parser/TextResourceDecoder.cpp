...
TextResourceDecoder::ContentType TextResourceDecoder::determineContentType(const String& mimeType)
{
    ...
    if (DOMImplementation::isXMLMIMEType(mimeType))
        return XMLContent;
#if defined(S_PLM_P141126_07375)
...
#if defined(S_PLM_P141126_07375)
        //To Do Check why WML documents encoding type is not correct.
        // for the time being for WML document let's go for detectTextEncoding
        if (shouldAutoDetect() || m_contentType == WMLContent) {
#else
        if (shouldAutoDetect()) {
#endif
