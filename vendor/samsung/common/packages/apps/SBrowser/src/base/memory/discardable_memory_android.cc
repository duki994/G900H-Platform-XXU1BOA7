...
struct DiscardableMemoryAllocatorWrapper {
 ...
 private:
  ...
  static size_t GetOptimalAshmemRegionSizeForAllocator() {
    ...
    // WAS: return base::android::SysUtils::AmountOfPhysicalMemoryKB() * 1024 / 8;
    // SAMSUNG: 354MB(in case of 3GB physical memory) is too big for 2GB virtual memory
    //                  Set max to 2GB
    return std::min(base::android::SysUtils::AmountOfPhysicalMemoryKB() * 1024 / 8, 
