...
const size_t kWorkerThreadStackSize = 128 * 1024;
#endif

#if defined(S_WORKER_POOL_PROFILE)
const int kBytesInKb = 1024;
const char kWorkerThreadStackSizeSwitch[] = "worker-thread-stack-size-in-kb";
size_t CalculateStackSize() {
    size_t stack_size = kWorkerThreadStackSize;
    if(CommandLine::ForCurrentProcess()->HasSwitch(kWorkerThreadStackSizeSwitch)) {
       std::string limit_string = CommandLine::ForCurrentProcess()->GetSwitchValueASCII(
    kWorkerThreadStackSizeSwitch);
       base::StringToSizeT(limit_string, &stack_size);
       stack_size = (stack_size*kBytesInKb < (size_t)kWorkerThreadStackSize) ? (size_t)kWorkerThreadStackSize : stack_size*kBytesInKb;
    }
    return stack_size;
}

size_t StackSize() {
  static size_t stack_size = CalculateStackSize();
  return stack_size;
}
#endif
...
// Comment unnecessary log out
//#if defined(S_WORKER_POOL_PROFILE)
//    LOG(INFO) << "[WorkerThread][ThreadMain][Run] On = " << name 
...
#if defined(S_WORKER_POOL_PROFILE)
    PlatformThread::CreateNonJoinable(StackSize(), worker);
#else
