...
scoped_ptr<DiscardableMemory> DiscardableMemoryAllocator::Allocate(
  ...
  // SAMSUNG : DiscardableMemory Limit >>
  // T Projects has only 2GB virtual address for each task.
  // And 1GB is consumed by system(dalvik, ...)
  // So virtual address sapce is not enough to allocate over 256 MB DiscardableAshmem
  // TODO(phoenix): Need to calculate the exact preserve ashmem size
  if (ashmem_regions_.size() * ashmem_region_size_ >= (256 * 1024 * 1024)) {
    return scoped_ptr<DiscardableMemory>();
  }
  // SAMSUNG <<
