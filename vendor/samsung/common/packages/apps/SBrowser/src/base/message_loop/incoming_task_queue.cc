...
TimeTicks IncomingTaskQueue::CalculateDelayedRuntime(TimeDelta delay) {
  ...
  if (delay > TimeDelta()) {
#if defined(S_PLM_P140816_02477)
    TimeTicks timeNow = TimeTicks::Now();
    int64 ticks = timeNow.ToInternalValue();
    const TimeTicks current_time = TimeTicks::FromInternalValue(ticks);	
    delayed_run_time = current_time + delay;
#else
