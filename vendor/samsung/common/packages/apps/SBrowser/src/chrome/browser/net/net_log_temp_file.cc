...
void NetLogTempFile::ProcessCommand(Command command) {
  ...
  switch (command) {
    ...
    case DO_START:
      StartNetLog(net::NetLog::LOG_ALL);
      break;
    case DO_START_ALL_BUT_BYTES:
      StartNetLog(net::NetLog::LOG_ALL_BUT_BYTES);
      break;
    case DO_START_BASIC:
      StartNetLog(net::NetLog::LOG_BASIC);
      break;
#else
    case DO_START:
      StartNetLog();
...
#if defined( S_NET_EXPORT_LOG )
void NetLogTempFile::StartNetLog(int logLevel) {
  DCHECK(BrowserThread::CurrentlyOn(BrowserThread::FILE_USER_BLOCKING));
  if (state_ == STATE_ALLOW_STOP)
    return;
  ...
  scoped_ptr<base::Value> constants(NetInternalsUI::GetConstants());
  net_log_logger_.reset(new net::NetLogLogger(file, *constants));
  net_log_logger_->StartObserving(chrome_net_log_, logLevel);
  state_ = STATE_ALLOW_STOP;
}
#endif
