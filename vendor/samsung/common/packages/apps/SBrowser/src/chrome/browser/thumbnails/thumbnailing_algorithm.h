...
class ThumbnailingAlgorithm
 ...
 public:
  ...
  typedef base::Callback<void(const ThumbnailingContext&,
      const GURL&, const SkBitmap&)> ConsumerCallbackWithURL;
#endif
...
#if defined(S_NATIVE_SUPPORT)
  virtual void ProcessBitmapWithURL(scoped_refptr<ThumbnailingContext> context,
                             const ConsumerCallbackWithURL& callback,
                             const GURL& gurl,
                             const SkBitmap& bitmap) = 0;
#endif
