...
class FaviconHandler {
 ...
 protected:
  ...
  // Notifies the delegate that the Touchicon for the active entry was updated.
  // |icon_url_changed| is true if a Touchicon with a different icon URL has been
  // selected since the previous call to NotifyTouchiconUpdated().
  virtual void NotifyTouchiconUpdated(bool icon_url_changed);
 ...
 private:
  ...
  void UpdateTouchicon(content::NavigationEntry* entry,
                     const GURL& icon_url,
                     const gfx::Image& image);
