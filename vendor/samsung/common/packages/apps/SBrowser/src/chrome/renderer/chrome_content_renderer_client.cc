...
#endif
  WebString content_scheme(ASCIIToUTF16(content::kContentScheme));
...
WebPlugin* ChromeContentRendererClient::CreatePlugin(
  ...
  if (status_value == ChromeViewHostMsg_GetPluginInfo_Status::kNotFound ||
...
#endif
#if defined(S_PLM_P140712_02056)
    ...
    PluginUMAReporter::GetInstance()->ReportPluginMissing(orig_mime_type, url);
    // Not creating the placeholder to force the renderer to draw the missing
    // plugin indicator.
    return NULL;
...
bool ChromeContentRendererClient::IsNaClAllowed(
  ...
  bool is_invoked_by_extension = false;
  bool is_nacl_pdf_viewer = false;
#endif
