...
#if defined(S_SAVE_PAGE_AS_HTML)
...
SavePackageFilePicker::SavePackageFilePicker(
  ...
  if (CommandLine::ForCurrentProcess()->HasSwitch(
      switches::kSavePageAsMHTML)) {
    suggested_path_copy =
        suggested_path_copy.Append(suggested_path.DirName().value() +
                                   "/" +
                                   GetTimestampString() +
                                   suggested_path.Extension());

    // File location prompting not implemented in android
    g_should_prompt_for_filename = false;
  } else {
      suggested_path_copy = suggested_path;
  }
...
const std::string SavePackageFilePicker::GetTimestampString() {
