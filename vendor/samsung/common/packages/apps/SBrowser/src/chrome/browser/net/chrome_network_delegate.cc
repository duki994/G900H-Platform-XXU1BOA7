...
#include "sbrowser/chrome/native/browser/android/sbr/net/sbr_url_filter.h"
#endif
#endif
...
int ChromeNetworkDelegate::OnBeforeURLRequest(
  ...
  if ((request->load_flags() & net::LOAD_MAIN_FRAME) != 0 &&
      net::SbrUrlFilterIsBlockedUrl(request->url().possibly_invalid_spec())) {
    request->net_log().AddEvent(
        net::NetLog::TYPE_CHROME_POLICY_ABORTED_REQUEST,
        net::NetLog::StringCallback("url",
                                    &request->url().possibly_invalid_spec()));
...
#else
  ForwardProxyErrors(request, profile_);
#endif
}
...
#if !defined(S_DISABLE_PERF_MONITOR)
