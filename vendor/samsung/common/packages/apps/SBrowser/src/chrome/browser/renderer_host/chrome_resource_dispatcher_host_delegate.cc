...
void ChromeResourceDispatcherHostDelegate::RequestBeginning(
  ...
  if (!is_prerendering && (resource_type == ResourceType::MAIN_FRAME 
						#if defined(SBROWSER_RTSP_IFRAME)
						|| (resource_type == ResourceType::SUB_FRAME && request->url().SchemeIs("rtsp"))																	
						#endif
  )) {
#if defined(OS_ANDROID)
    throttles->push_back(
...
#if defined(FULL_SAFE_BROWSING) || defined(MOBILE_SAFE_BROWSING) || defined(ENABLE_MANAGED_USERS)
