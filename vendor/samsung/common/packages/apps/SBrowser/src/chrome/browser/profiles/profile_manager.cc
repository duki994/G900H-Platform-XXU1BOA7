...
base::FilePath ProfileManager::GetLastUsedProfileDir(
  ...
  VLOG(0) << __FUNCTION__ << " kProfileLastUsed = "
      << local_state->FindPreference(prefs::kProfileLastUsed);
  if (local_state->HasPrefPath(prefs::kProfileLastUsed)) {
    VLOG(0) << " Got kProfileLastUsed = "
        << local_state->GetString(prefs::kProfileLastUsed);
    return last_used_profile_dir.AppendASCII(
        local_state->GetString(prefs::kProfileLastUsed));
  }

  VLOG(0) << " Returning kProfileLastUsed = " << chrome::kInitialProfile;
...
void ProfileManager::ScheduleProfileForDeletion(
  ...
  VLOG(0) << __FUNCTION__ << " Comparing with MaybeAsASCII kProfileLastUsed = "
      << local_state->GetString(prefs::kProfileLastUsed);
  if (profile_dir.BaseName().MaybeAsASCII() ==
    ...
    if (last_non_managed_profile.empty()) {
      ...
      VLOG(0) << " Setting with MaybeAsASCII kProfileLastUsed = "
      ...
      VLOG(0) << " Setting with last_non_managed_profile kProfileLastUsed = "
...
void ProfileManager::BrowserListObserver::OnBrowserSetLastActive(
  ...
  if (profile_manager_->profiles_info_.find(
    ...
    VLOG(0) << __FUNCTION__ << " setting with MaybeAsASCII kProfileLastUsed = "
