...
#endif
#if defined(S_NETWORK_CONNECTION_COUNT)
...
#if defined(S_NETWORK_CONNECTION_COUNT)
  void SetMaxNumDelayableRequestsPerClient(const base::ListValue* list);
  void SetMaxNumDelayableRequestsPerHost(const base::ListValue* list);
  void SetMaxSocketsPerGroup(const base::ListValue* list);
#endif
...
#if defined(S_NETWORK_CONNECTION_COUNT)
  web_ui()->RegisterMessageCallback(
      "setMaxNumDelayableRequestsPerClient",
      base::Bind(&IOThreadImpl::CallbackHelper,
                 &IOThreadImpl::SetMaxNumDelayableRequestsPerClient, proxy_));
  web_ui()->RegisterMessageCallback(
      "setMaxNumDelayableRequestsPerHost",
      base::Bind(&IOThreadImpl::CallbackHelper,
                 &IOThreadImpl::SetMaxNumDelayableRequestsPerHost, proxy_));
  web_ui()->RegisterMessageCallback(
      "setMaxSocketsPerGroup",
      base::Bind(&IOThreadImpl::CallbackHelper,
                 &IOThreadImpl::SetMaxSocketsPerGroup, proxy_));
#endif
  web_ui()->RegisterMessageCallback(
      "clearBrowserCache",
...
#else
  SendJavascriptCommand("receivedSpdySessionInfo", NULL);
#endif
}
...
#else
  SendJavascriptCommand("receivedQuicInfo", NULL);
#endif
}
...
#if defined(S_NETWORK_CONNECTION_COUNT)
void NetInternalsMessageHandler::IOThreadImpl::SetMaxNumDelayableRequestsPerClient(const base::ListValue* list) {
  int num = 0;
  std::string num_string;
  if (!list->GetString(0, &num_string) ||
      !base::StringToInt(num_string, &num)) {
    LOG(INFO) << "yongje : SetMaxNumDelayableRequestsPerClient() invalid input";
    return;
  }
  LOG(INFO) << "yongje : SetMaxNumDelayableRequestsPerClient() num:" << num;
  content::ResourceScheduler::SetMaxNumDelayableRequestsPerClient(num);
}

void NetInternalsMessageHandler::IOThreadImpl::SetMaxNumDelayableRequestsPerHost(const base::ListValue* list) {
  int num = 0;
  std::string num_string;
  if (!list->GetString(0, &num_string) ||
      !base::StringToInt(num_string, &num)) {
    LOG(INFO) << "yongje : SetMaxNumDelayableRequestsPerHost() invalid input";
    return;
  }
  LOG(INFO) << "yongje : SetMaxNumDelayableRequestsPerHost() num:" << num;
  content::ResourceScheduler::SetMaxNumDelayableRequestsPerHost(num);
}

void NetInternalsMessageHandler::IOThreadImpl::SetMaxSocketsPerGroup(const base::ListValue* list) {
  // not implemented
}
#endif
}  // namespace
