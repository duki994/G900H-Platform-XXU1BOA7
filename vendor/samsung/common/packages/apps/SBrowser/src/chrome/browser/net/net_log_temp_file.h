...
class NetLogTempFile {
 public:
  ...
  enum Command {
    ...
    DO_START_ALL_BUT_BYTES, // Call StartLog with LOG_ALL_BUT_BYTES.
    DO_START_BASIC, // Call StartLog with LOG_BASIC.
#endif
...
#if defined( S_NET_EXPORT_LOG )
  // Start collecting NetLog data with net::NetLog::LogLevel
  // into chrome-net-export-log.json file in
  // base::GetTempDir() directory. It is a no-op if we are already
  // collecting data into a file.
  void StartNetLog(int logLevel);
#endif
