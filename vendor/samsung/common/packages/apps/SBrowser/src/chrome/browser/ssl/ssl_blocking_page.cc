...
std::string SSLBlockingPage::GetHTMLContents() {
  ...
  if (overridable_ && !strict_enforcement_) {
...
#if defined(S_SSL_ERROR)
    resource_id = IDR_SBR_SSL_BLOCKING_HTML;
#else
...
#if defined(S_SSL_ERROR)
    strings.SetString(
        "message", l10n_util::GetStringFUTF16(IDS_SBR_SSL_BLOCKING_PAGE_BODY_TEXT,
                                              url.c_str()));
    strings.SetString(
        "moreMessage",
        l10n_util::GetStringFUTF16(IDS_SBR_SSL_BLOCKING_PAGE_MORE_TEXT,
                                   url.c_str()));
#else
    strings.SetString(
    ...
    if (type == SSLErrorInfo::CERT_REVOKED) {
      ...
      strings.SetString(
          "failure",
          l10n_util::GetStringFUTF16(IDS_SBR_SSL_BLOCKING_PAGE_HSTS, url.c_str()));
    }
#else 
    if (type == SSLErrorInfo::CERT_REVOKED) {
      errorType = base::string16(ASCIIToUTF16("Key revocation"));
