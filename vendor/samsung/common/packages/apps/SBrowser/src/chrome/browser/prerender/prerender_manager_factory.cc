...
BrowserContextKeyedService* PrerenderManagerFactory::BuildServiceInstanceFor(
...
#if defined(S_PLM_P140718_08958)
  if (!profile || !(g_browser_process->prerender_tracker()))
  	return NULL;
#endif
#if defined(OS_ANDROID)
  if (base::android::SysUtils::IsLowEndDevice())
    return NULL;
#endif
  LOG(INFO) << "[Chrome Native] profile: " << profile;
  LOG(INFO) << "[Chrome Native] profile->GetPrefs(): " << profile->GetPrefs();
