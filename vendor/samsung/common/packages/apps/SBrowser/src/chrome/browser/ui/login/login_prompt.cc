...
#if defined(SBROWSER_KERBEROS_FEATURE)
int LoginHandler::IsNegotiatePresent() {
  int iState = 0;
  std::string scheme;
  if (request_->GetResponseCode() == 401) {
    request_->GetResponseHeaderByName("WWW-Authenticate",&scheme);
  } else if (request_->GetResponseCode() == 407) {
    request_->GetResponseHeaderByName("Proxy-Authenticate",&scheme);
  } else {
    LOG(ERROR) << "IsNegotiatePresent() can't find auth header";
    return -1;
  }
...
void LoginHandler::CloseContentsDeferred() {
  ...
  void LoginHandler::BuildViewForPasswordManager(
      PasswordManager* manager,
      const base::string16& explanation,
      bool is_proxy_auth,
      bool did_use_http_auth) {
    BuildViewForPasswordManager(manager, explanation);
  }
#endif
...
void LoginDialogCallback(const GURL& request_url,
  ...
  handler->BuildViewForPasswordManager(password_manager, explanation,
      auth_info->is_proxy, auth_info->did_use_http_auth);
#else
  handler->BuildViewForPasswordManager(password_manager, explanation);
#endif
}
