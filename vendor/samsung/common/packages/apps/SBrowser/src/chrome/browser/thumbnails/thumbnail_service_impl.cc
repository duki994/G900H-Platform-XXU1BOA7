...
bool ThumbnailServiceImpl::SetPageThumbnailWithURL(const ThumbnailingContext& context,
                                      const GURL& gurl,
                                      const gfx::Image& thumbnail) {
  scoped_refptr<history::TopSites> local_ptr(top_sites_);
  if (local_ptr.get() == NULL)
  ...
  GURL url = GURL();
  if(local_ptr->IsBlacklistedFromRedirect(context.url, &url)) {
    if(!url.is_empty())
      local_ptr->RemoveBlacklistedURL(url);
  }

  return local_ptr->SetPageThumbnail(gurl, thumbnail, context.score);
}
#endif
...
bool ThumbnailServiceImpl::ShouldAcquirePage(const GURL& url) {
  ...
  if (!HistoryService::CanAddURL(url))
    return false;
  // Skip if the top sites list is full, and the URL is not known.
  if (local_ptr->IsForcedFull() && !local_ptr->IsKnownURL(url))
    return false;
