...
#if !defined(OS_CHROMEOS) && defined(ENABLE_SIGNIN)
...
// Bing Search Engine Changes Start
#include "sbrowser/chrome/native/browser/android/sbr/preferences/sbr_pref_names.h"
#include "sbrowser/chrome/native/browser/android/sbr/sbrowser_content_browser_client_delegate.h"
// Bing Search Engine Changes End
#endif
...
void ChromeContentBrowserClient::GetStoragePartitionConfigForSite(
  ...
  *in_memory = false;
  ...
  if (site.GetOrigin().spec() == kChromeUIChromeSigninURL) {
#endif
    // Chrome signin page has an embedded iframe of extension and web content,
    // thus it must be isolated from other webUI pages.
...
#if defined(OS_POSIX) && !defined(S_DISABLE_BREAKPAD)
...
void ChromeContentBrowserClient::RequestDesktopNotificationPermission(
    ...
    int render_process_id,
    int render_view_id) {
#if defined(ENABLE_NOTIFICATIONS) && defined(ENABLE_EXTENSIONS_ALL) 
  DCHECK(BrowserThread::CurrentlyOn(BrowserThread::UI));
...
// Bing Search Engine Changes End
// Save page feature
  web_prefs->allow_content_url_access =
      prefs->GetBoolean(prefs::kWebKitAllowContentURLAccess);
  // For Font Boosting Feature
  web_prefs->fontboosting_mode_enabled =
          prefs->GetBoolean(prefs::kWebKitFontBoostingModeEnabled);
  #if defined(S_AUTOCOMPLETE_IGNORE)
      web_prefs->autocomplete_ignore=
          prefs->GetBoolean(prefs::kWebKitAutocompleteIgnore);
  #endif

  web_prefs->text_autosizing_enabled = web_prefs->fontboosting_mode_enabled;

  rvh->SetTextZoomFactor(web_prefs->font_scale_factor);
  ...
  web_prefs->initialize_at_minimum_page_scale =
      prefs->GetBoolean(prefs::kWebkitOverviewModeEnabled);
...
#endif
#endif //ENABLE_EXTENSIONS_ALL

#if defined(SBROWSER_CSC_FEATURE)
          if( base::android::sbr::getEnableStatus( "CscFeature_Web_Bool_Disable_User_Gesture_required" ))
...
#if defined(ENABLE_FILE_SYSTEM_SYNC)
...
#if defined(S_NATIVE_SUPPORT) // Bing Search Engine changes
void ChromeContentBrowserClient::SetBingAsCurrentSearchDefault(RenderViewHost* rvh) {
  WebContents* web_contents = WebContents::FromRenderViewHost(rvh);
  SbrowserContentBrowserClientDelegate::SetBingAsCurrentSearchEngine(web_contents);
}
#endif
