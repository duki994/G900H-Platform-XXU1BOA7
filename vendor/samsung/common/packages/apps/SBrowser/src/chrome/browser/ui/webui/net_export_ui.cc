...
#if defined( S_NET_EXPORT_LOG )  
  source->AddResourcePath("sbr_net_export.js", IDR_SBR_NET_EXPORT_JS);
  source->SetDefaultResource(IDR_SBR_NET_EXPORT_HTML);
#else
...
#if defined( S_NET_EXPORT_LOG )
  void OnStartNetLogAllButBytes(const base::ListValue* list);
  void OnStartNetLogBasic(const base::ListValue* list);
#endif  
...
#if defined( S_NET_EXPORT_LOG )
    web_ui()->RegisterMessageCallback(
        "startNetLogAllButBytes",
        base::Bind(&NetExportMessageHandler::OnStartNetLogAllButBytes,
                   base::Unretained(this)));
    web_ui()->RegisterMessageCallback(
        "startNetLogBasic",
        base::Bind(&NetExportMessageHandler::OnStartNetLogBasic,
                   base::Unretained(this)));
#endif  
  web_ui()->RegisterMessageCallback(
      "stopNetLog",
...
#if defined( S_NET_EXPORT_LOG )
void NetExportMessageHandler::OnStartNetLogAllButBytes(const base::ListValue* list) {
  ProcessNetLogCommand(weak_ptr_factory_.GetWeakPtr(),
                       net_log_temp_file_,
                       NetLogTempFile::DO_START_ALL_BUT_BYTES);
}

void NetExportMessageHandler::OnStartNetLogBasic(const base::ListValue* list) {
  ProcessNetLogCommand(weak_ptr_factory_.GetWeakPtr(),
                       net_log_temp_file_,
                       NetLogTempFile::DO_START_BASIC);
}
#endif
