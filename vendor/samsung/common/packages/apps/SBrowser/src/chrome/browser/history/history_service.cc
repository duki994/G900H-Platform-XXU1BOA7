...
bool HistoryService::CanAddURL(const GURL& url) {
  ...
  int reading_list_URI_Length = std::string(chrome::kReadingListPageURI).length();
  if (url.spec().compare(0, reading_list_URI_Length, chrome::kReadingListPageURI) == 0) {
    VLOG(1) << "Cannot add ReadingList URI in history returning false";
    return false;
  }
...
#if defined(ENABLE_SIGNIN) && defined(ENABLE_SYNC)
