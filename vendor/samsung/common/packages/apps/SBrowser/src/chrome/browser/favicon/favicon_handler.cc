...
void FaviconHandler::SetFavicon(
  ...
  if (UrlMatches(url, url_)) {
    NavigationEntry* entry = GetEntry();
    if (entry){
      if(icon_type == chrome::FAVICON)
        UpdateFavicon(entry, icon_url, image);
      else if(icon_type == chrome::TOUCH_ICON)
        UpdateTouchicon(entry, icon_url, image);
    }
  }
}
...
void FaviconHandler::UpdateTouchicon(NavigationEntry* entry,
                                   const GURL& icon_url,
                                   const gfx::Image& image) {//write entry class copy
  // No matter what happens, we need to mark the touchicon as being set.
  entry->GetFavicon().valid = true;
  ...
  entry->GetFavicon().image = image_with_adjusted_colorspace;
  NotifyTouchiconUpdated(icon_url_changed);
}
...
void FaviconHandler::NotifyTouchiconUpdated(bool icon_url_changed) {
  delegate_->NotifyTouchiconUpdated(icon_url_changed);//write NotifyTouchiconUpdated() in delegate_'s class
}
