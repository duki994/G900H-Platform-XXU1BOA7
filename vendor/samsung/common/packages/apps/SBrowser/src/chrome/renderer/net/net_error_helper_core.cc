...
void NetErrorHelperCore::GenerateLocalErrorPage(
  ...
  LOG(INFO) << "error.reason : " << error.reason;
#endif
  if (frame_type == MAIN_FRAME) {
    pending_error_page_info_.reset(new ErrorPageInfo(error, is_failed_post));
    if (IsDnsError(error)) {
...
#else
      delegate_->GenerateLocalizedErrorPage(
          GetUpdatedError(error), is_failed_post, error_html, error.reason);
#endif
...
#else
  delegate_->GenerateLocalizedErrorPage(error, is_failed_post, error_html, error.reason);
#endif
}
