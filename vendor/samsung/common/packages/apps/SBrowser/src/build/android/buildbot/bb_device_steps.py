...
def ForceBatteryStatusSafe(device):
  """Force the device to think that a battery is attached."""
  try:
    android_commands.AndroidCommands(device).ForceBatteryStatus()
  except errors.DeviceUnresponsiveError as e:
    return e
...
def GetDeviceStepsOptParser():
  ...
  parser.add_option('--force-battery-status', action='store_true',
                    help='Force battery status (if no battery is connected).')
