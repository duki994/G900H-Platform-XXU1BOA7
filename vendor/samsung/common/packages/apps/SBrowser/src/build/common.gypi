...
# since gyp_chromium is automatically forcing its inclusion.
{
  ...
  'variables': {
    ...
    'variables': {
      ...
      # By default, use static lib. For low end target, system lib is used.
      'use_system_libpng%': 0,
      ...
      # Enable low end patches forcibly which is disabled by default.
      'enable_low_end_feature%': 0,
      'enable_4444_texture%': 0,
      ...
      # Keep the dev_tools code as per original
      'enable_devtools%': 1,
      ...
      # Enables autofill. This is disabled in low end devices
      'enable_autofill%': 1,

      # Enables edmaudit (KNOX requirement). This is disabled in KNOX-less devices
      'enable_edmaudit%': 1,

      #Crash is enabled for High end devices.
      'enable_crashes%': 1,

      #Disabling Feedback in SBrowser.
      'enable_feedback%': 0,

      #Flag for enabling/disabling mostvisited in SBrowser.
      'enable_mostvisited%': 1,

      #Disabling GCM in SBrowser.
      'enable_gcm%': 0,

      #Disables building of webgl. Only switched on for low end devices
      'disable_webgl%': 0,

      #Disables building of libphonenumber. Only switched on for low end devices
      'disable_libphonenumber%': 0,

      #Disables building of autocomplete. Only switched on for low end devices
      ...
      # Enables prerender. This is disabled in low end devices
      'enable_prerender%': 1,

      # Enables angle (used for webgl shader translation/validation). This is disabled in low end devices if webgl is disabled
      'disable_angle%': 0,

      #Disables building of webgl. Only switched on for low end devices
      'disable_libwebp%': 0,

      # Enables web ui omnibox 'chrome://omnibox'. This is disabled in low end devices
      'enable_webui_omnibox%': 1,
    ...
    'use_system_skia%': '<(use_system_skia)',
    ...
    'enable_edmaudit%': '<(enable_edmaudit)',
    'enable_crashes%': '<(enable_crashes)',
    'enable_feedback%': '<(enable_feedback)',
    'enable_mostvisited%': '<(enable_mostvisited)',
    'enable_gcm%': '<(enable_gcm)',
    'enable_translate%': '<(enable_translate)',
    'enable_prerender%': '<(enable_prerender)',
    'disable_angle%': '<(disable_angle)',
    'disable_webgl%': '<(disable_webgl)',
    'disable_libphonenumber%': '<(disable_libphonenumber)',
    'disable_autocomplete%': '<(disable_autocomplete)',
    'disable_spdy_quic%': '<(disable_spdy_quic)',
    'disable_libwebp%': '<(disable_libwebp)',
    'enable_webui_omnibox%': '<(enable_webui_omnibox)',
    ...
    'enable_file_system_sync%': '<(enable_file_system_sync)',
    ...
    'use_system_libpng%': '<(use_system_libpng)',
    'use_system_harfbuzz_ng%': '<(use_system_harfbuzz_ng)',
    ...
    'use_goma%': '<(use_goma)',
    'gomadir%': '<(gomadir)',
    'enable_low_end_feature%': '<(enable_low_end_feature)',
    'enable_4444_texture%': '<(enable_4444_texture%)',
    ...
    'conditions': [
      ...
      ['OS=="android"', {
        ...
        'android_sdk_jar%': '<(android_sdk)/samsung-android.jar',
      ...
      ['enable_webui_omnibox==1', {
        'grit_defines': ['-D', 'enable_webui_omnibox'],
      }],
      ['enable_mdns==1', {
  ...
  'target_defaults': {
    ...
    'conditions': [
      ...
      ['enable_gpu_rasterization==1', {
        'defines': ['SBROWSER_GPU_RASTERIZATION_ENABLE=1'],
      }],
      ['enable_webrtc==1', {
        'defines': ['ENABLE_WEBRTC=1'],
      }],
      ['use_system_skia==1', {
        'defines': ['USE_SYSTEM_SKIA=1'],
      }],
      ['enable_devtools==1', {
        'defines': ['ENABLE_DEV_TOOLS=1'],
      }],      
      ['proprietary_codecs==1', {
      ...
      ['enable_autofill==1', {
        'defines': ['ENABLE_AUTOFILL=1'],
      }],
      ['enable_crashes==1', {
        'defines': ['ENABLE_CRASHES=1'],
      }],
      ['enable_feedback==1', {
        'defines': ['ENABLE_FEEDBACK=1'],
      }],
      ['enable_mostvisited==1', {
        'defines': ['ENABLE_MOSTVISITED=1'],
      }],
      ['enable_gcm==1', {
        'defines': ['ENABLE_GCM=1'],
      }],
      ['enable_translate==1', {
        'defines': ['ENABLE_TRANSLATE=1'],
      }],
      ['enable_prerender==1', {
        'defines': ['ENABLE_PRERENDER=1'],
      }],
      ['use_system_freetype==1', {
        'defines': ['USE_SYSTEM_FREETYPE=1'], #disable FSN_FONT_SOLUTION CL 1629391
      }],
      ['disable_angle==1', {
        'defines': ['DISABLE_ANGLE=1'],
      }],
      ['disable_webgl==1', {
        'defines': ['DISABLE_WEBGL=1'],
      }],
      ['disable_libphonenumber==1', {
        'defines': ['DISABLE_LIBPHONENUMBER=1'],
      }],
      ['disable_autocomplete==1', {
        'defines': ['DISABLE_AUTOCOMPLETE=1'],
      }],
      ['disable_spdy_quic==1', {
        'defines': ['DISABLE_SPDY_QUIC=1'],
      }],
      ['disable_libwebp==1', {
        'defines': ['DISABLE_LIBWEBP=1'],
      }],
      ['enable_webui_omnibox==1', {
        'defines': ['ENABLE_WEBUI_OMNIBOX=1'],
      }],
      ['enable_background==1', {
      ...
      ['enable_file_system_sync==1', {
        'defines': ['ENABLE_FILE_SYSTEM_SYNC=1'],
      }],
      ['enable_spellcheck==1', {
      ...
      ['enable_low_end_feature==1', {
        'defines': ['ENABLE_LOW_END_FEATURE=1'],
      }],
      ['enable_4444_texture==1', {
        'defines': ['ENABLE_4444_TEXTURE=1'],
      }],
      ['OS=="win" and component=="shared_library"', {
        'dependencies': [
  ...
  'conditions': [
              ...
              ['OS=="android"', {
                ...
                'conditions' : [
                  ['use_system_libjpeg == 0', {
                    'ldflags': [
                      '-Wl,--warn-shared-textrel',
                      '-Wl,--no-fatal-warnings', # ignore linking time warnings as error
                    ],
                    }, {
                     'ldflags': [
                       '-Wl,--warn-shared-textrel',
                      # '-Wl,--fatal-warnings', # ignore linking time warnings as error
                       ],
                    }],
                  ],
              }],
          ...
          ['_toolset=="target"', {
            ...
            'conditions': [
              ...
              ['enable_edmaudit==1', {
                ...
                'cflags': [
                  '-I<(android_src)/frameworks/native/include',
                ],
                'ldflags': [
                  '-L<(android_libs_dir)',
                  '-Wl,--no-fatal-warnings',
    ...
    ['OS=="android" and clang==0', {
      ...
      'make_global_settings': [
        ['CC', '<!(/bin/echo -n $CCACHE_EXE <(android_toolchain)/*-gcc)'],
        ['CXX', '<!(/bin/echo -n $CCACHE_EXE <(android_toolchain)/*-g++)'],
        ['CC.host', '<!(which gcc)'],
        ['CXX.host', '<!(which g++)'],
      ],
    }],
