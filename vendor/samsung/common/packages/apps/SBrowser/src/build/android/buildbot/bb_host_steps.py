...
def CopySBrowserAPK(_options):
  bb_annotations.PrintNamedStep('copy_sbrowser_apk')
  sbrowser_dir = os.path.join(DIR_BUILD_ROOT, 'src', 'sbrowser')
  shutil.copy(os.path.join(sbrowser_dir, 'app', 'bin', 'SBrowser-release.apk'),
              os.path.join(DIR_BUILD_ROOT, 'src', 'out', 'Release', 'apks'))


def LinkPlatformDir(_options):
  bb_annotations.PrintNamedStep('link_platform_dir')

  platform_path = os.path.join(DIR_BUILD_ROOT, 'platform')
  if not os.path.isdir(platform_path):
    # Cannot find platform dir: we are probably running from buildbot
    # Need to make a symlink so SBrowser apk build finds platform jars
    if os.path.islink(platform_path) or os.path.exists(platform_path):
      os.unlink(platform_path)
    os.symlink('trunk/platform', platform_path)
...
def GetHostStepCmds():
  return [
      ('link_platform_dir', LinkPlatformDir),
      ('compile', Compile),
      ('copy_sbrowser_apk', CopySBrowserAPK),
