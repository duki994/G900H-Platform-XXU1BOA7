...
def GetEnvironment(host_obj, testing, extra_env_vars=None):
  ...
  env['GYP_DEFINES'] = env.get('GYP_DEFINES', '') + \
      ' fastbuild=1 use_goma=0 gomadir=%s' % bb_utils.GOMA_DIR
...
def GetCommands(options, bot_config):
  ...
  if test_obj:
    run_test_cmd = [test_obj.script, '--no-reboot',
        '--force-battery-status'] + property_args
...
def GetBotStepMap():
  ...
  bot_configs = [
      ...
      B('perf-builder-rel',
          H(['link_platform_dir', 'compile', 'copy_sbrowser_apk', 'zip_build'],
          extra_args=['--build-targets=sbrowser_release_apk'],
          extra_gyp='enable_s_android_browser=1 enable_s=1')),
      B('perf-tests-rel-note3', H(std_test_steps),
        T([], ['--install=SBrowser'])),
  ]
