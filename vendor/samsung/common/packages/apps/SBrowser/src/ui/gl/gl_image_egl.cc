...
#if defined(OS_ANDROID)
  if (buffer.type == gfx::EGL_CLIENT_BUFFER_SEC) {
    int width = buffer.flattened_buffer[0];
    int height = buffer.flattened_buffer[1];

    gpu_buffer = new content::GpuMemoryBufferImplAndroidSEC(gfx::Size(width, height), GL_RGBA8_OES);
    if (gpu_buffer && gpu_buffer->InitializeFromClientSide(buffer))
      buffer.native_buffer = reinterpret_cast<EGLClientBuffer>(gpu_buffer->GetNativeBuffer());
  }
#endif
