...
NotificationList::PopupNotifications NotificationList::GetPopupNotifications(
  ...
  /*
   *These lines of code is used for message center ui, which is not useful for
   * Android so we are working on removing message_center from build , This will
   * reduce the size of library also.
   */
  for (Notifications::reverse_iterator iter = notifications_.rbegin();
