...
#include "out_jni/Clipboard_jni.h"
#else
#include "out_jni/SbrClipboard_jni.h"
#endif
...
class ClipboardMap {
 ...
 private:
  ...
  base::Lock lock_;
#if defined (S_NATIVE_SUPPORT) // SBROWSER_CLIPBOARDEX_IMPL
  void printClipboardMapData();
#endif

  // Java class and methods for the Android ClipboardManager.
  ScopedJavaGlobalRef<jobject> clipboard_manager_;
#if defined (S_NATIVE_SUPPORT) // SBROWSER_CLIPBOARDEX_IMPL
  ScopedJavaGlobalRef<jobject> sbr_clipboard_manager_;
#endif
};
...
ClipboardMap::ClipboardMap() {
  ...
  ScopedJavaLocalRef<jobject> local_ref =
    Java_SbrClipboard_create(env, context);
  DCHECK(local_ref.obj());
  sbr_clipboard_manager_.Reset(env, local_ref.Release());
#else
  ScopedJavaLocalRef<jobject> local_ref =
...
#if defined(S_NATIVE_SUPPORT) //SBROWSER_CLIPBOARDEX_IMPL
    Java_SbrClipboard_setText(env, sbr_clipboard_manager_.obj(), str.obj());
#else
...
#if defined(S_NATIVE_SUPPORT) //SBROWSER_CLIPBOARDEX_IMPL
    Java_SbrClipboard_setHTML(env, sbr_clipboard_manager_.obj(), html.obj());
#else
...
#if defined(S_NATIVE_SUPPORT) //SBROWSER_CLIPBOARDEX_IMPL
  java_string_text =
   Java_SbrClipboard_getCoercedText(env, sbr_clipboard_manager_.obj());
#else
  java_string_text =
...
#if defined(S_NATIVE_SUPPORT) //SBROWSER_CLIPBOARDEX_IMPL
  if (!Java_SbrClipboard_isHTMLClipboardSupported(env)) {
    return;
  }
#else
...
#if defined(S_NATIVE_SUPPORT) //SBROWSER_CLIPBOARDEX_IMPL
  java_string_html =
    Java_SbrClipboard_getHTMLText(env, sbr_clipboard_manager_.obj());
#else
  java_string_html =
...
#if defined(S_NATIVE_SUPPORT) //SBROWSER_CLIPBOARDEX_IMPL
void ClipboardMap::printClipboardMapData() {
  std::map<std::string, std::string>::const_iterator it =
    map_.begin();
  while(it != map_.end()) {
    ++it;
...
#if defined(S_NATIVE_SUPPORT)
bool RegisterSbrClipboardAndroid(JNIEnv* env) {
  return RegisterNativesImpl(env);
}
#endif
