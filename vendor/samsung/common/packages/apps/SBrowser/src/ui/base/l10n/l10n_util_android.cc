...
//TODO: This is a temporary patch for power optimization.
#if defined(SBROWSER_LOCALE_OPTIMIZATION)
  static std::string prev_locale;
  static std::string prev_display_locale;
  static base::string16 prev_result;
  if(prev_locale == locale && prev_display_locale == display_locale)
    return prev_result;
  prev_locale = locale;
  prev_display_locale = display_locale;
#endif
  JNIEnv* env = base::android::AttachCurrentThread();
...
#if defined(SBROWSER_LOCALE_OPTIMIZATION)
  prev_result = ConvertJavaStringToUTF16(java_result);
  return prev_result;
#else
