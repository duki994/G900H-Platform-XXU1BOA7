...
// gralloc usually uses 2, 3 or more file descriptor.
// and it depends on graphic chipset vendors implementation.
// However gralloc would uses no more than 4 normally.
#if defined(OS_ANDROID)
const int gpu_memory_buffer_handle_size = 4;
#endif
...
struct GpuMemoryBufferHandle {
  ...
  {
#if defined(OS_ANDROID)
    handle_fd[0] = handle_fd[1] =
    handle_fd[2] = handle_fd[3] = base::SharedMemory::NULLHandle();
#endif
  }
  ...
  base::SharedMemoryHandle handle_fd[gpu_memory_buffer_handle_size];
  std::vector<int> flattened_buffer;
#endif
#if defined(OS_MACOSX)
