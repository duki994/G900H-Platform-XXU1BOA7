...
float SharedDeviceDisplayInfo::GetPhysicalDisplayInch() {
  base::AutoLock autolock(lock_);
  return physical_display_inch_;
}
...
SharedDeviceDisplayInfo::SharedDeviceDisplayInfo()
  ...
  UpdateDisplayInfo(env, j_device_info_.obj(),
      ...
      Java_DeviceDisplayInfo_calcDeviceInch(env, j_device_info_.obj()),
...
void SharedDeviceDisplayInfo::UpdateDisplayInfo(JNIEnv* env,
  ...
  physical_display_inch_ = static_cast<float>(physical_device_inch);
