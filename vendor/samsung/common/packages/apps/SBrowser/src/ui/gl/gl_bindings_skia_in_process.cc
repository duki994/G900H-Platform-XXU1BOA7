...
GrGLInterface* CreateInProcessSkiaGLBinding() {
  ...
  //functions->fDiscardFramebuffer = StubGLDiscardFramebuffer; //KKULEN (needed???)
  ...
  //functions->fInsertEventMarker = StubGLInsertEventMarker; //KKULEN (needed???)
  functions->fLineWidth = StubGLLineWidth;
  functions->fLinkProgram = StubGLLinkProgram;
  functions->fPixelStorei = StubGLPixelStorei;
  //functions->fPopGroupMarker = StubGLPopGroupMarker;  //KKULEN (needed???)
  //functions->fPushGroupMarker = StubGLPushGroupMarker;  //KKULEN (needed???)
