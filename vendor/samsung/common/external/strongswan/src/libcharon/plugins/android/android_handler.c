...
struct private_android_handler_t {
	...
	/**
 	* Android service class to pass DNS info.
	*/
	android_service_t *android_service;
};
	...
	configuration_attribute_type_t type, chunk_t data)
{
	switch (type)
	{
		...
		{			
			...
			DBG1(DBG_CFG, "DNS Address handle: %H", dns);
			if (dns)
			{
			  /*
			  ...
			  */
			  this->android_service->set_dns(this->android_service,
							 dns);
				return TRUE;
			}
			return FALSE;
 ...
 * See header
 */
android_handler_t *android_handler_create(bool frontend, android_service_t *android_service)
{
	private_android_handler_t *this;
